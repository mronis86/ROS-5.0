...
class OSCGUIApp:
    ...
    def setup_events_tab(self, parent):
        ...
        auto_check = ttk.Checkbutton(auto_frame, text="Auto-refresh schedule (3s)", variable=self.auto_refresh_var, command=self.toggle_auto_refresh)
...
6. /subtimer/cue/5/start          - Start sub-timer for cue 5 on day 2"""
    ...
    def start_auto_refresh(self):
        """Start automatic schedule refresh every 3 seconds"""
        if hasattr(self, 'current_event_id') and self.current_event_id:
            self.auto_refresh_timer = self.root.after(3000, self.auto_refresh_schedule)  # 3 seconds
            self.schedule_status_var.set("Auto-refresh ON - checking every 3s")
            self.log_message("🔄 Auto-refresh enabled - will check for schedule changes every 3 seconds")
        else:
    ...
    def auto_refresh_schedule(self):
        ...
        if hasattr(self, 'current_event_id') and self.current_event_id and self.auto_refresh_var.get():
            try:
                # Store current schedule data for comparison
                old_schedule = self.schedule_data.copy() if hasattr(self, 'schedule_data') else []
                ...
                if response.data:
                    ...
                    # Check for changes in count, items, or durations
                    has_changes = False
                    change_details = []
                    ...
                    if new_count != old_count:
                        has_changes = True
                        change_details.append(f"count: {old_count} → {new_count}")
                    
                    # Check for changes in existing items (including durations)
                    if not has_changes and old_count > 0:
                        for i, new_item in enumerate(new_schedule):
                            if i < len(old_schedule):
                                old_item = old_schedule[i]
                                
                                # Check for duration changes
                                old_duration = f"{old_item.get('durationHours', 0)}:{old_item.get('durationMinutes', 0)}:{old_item.get('durationSeconds', 0)}"
                                new_duration = f"{new_item.get('durationHours', 0)}:{new_item.get('durationMinutes', 0)}:{new_item.get('durationSeconds', 0)}"
                                
                                if old_duration != new_duration:
                                    has_changes = True
                                    change_details.append(f"duration change in item {i+1}")
                                    break
                                
                                # Check for other field changes
                                for field in ['segmentName', 'shotType', 'programType', 'notes', 'assets']:
                                    if old_item.get(field) != new_item.get(field):
                                        has_changes = True
                                        change_details.append(f"{field} change in item {i+1}")
                                        break
                    ...
                    if has_changes:
                        ...
                        self.log_message(f"🔄 Schedule auto-updated! Changes: {', '.join(change_details)}")
                        self.schedule_status_var.set(f"Updated! {new_count} items")
                    else:
                        self.schedule_status_var.set(f"Synced - {new_count} items")
                
                # Schedule next check (3 seconds)
                if self.auto_refresh_var.get():
                    self.auto_refresh_timer = self.root.after(3000, self.auto_refresh_schedule)
