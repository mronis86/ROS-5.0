...
const EventListPage: React.FC = () => {
  ...
  // Load events from LAN SQLite on mount
  useEffect(() => {
  ...
  const loadEventsFromSupabase = async () => {
    try {
      ...
      console.log('🔄 Loading events from offline LAN database...');
      ...
      if (calendarEvents && calendarEvents.length > 0) {
        ...
        const eventsFromSupabase = calendarEvents.map((calEvent, index) => {
          ...
          const isQm = isQuickModeCalendarEvent(calEvent);
          const eventId = isQm
            ? (calEvent.id || Date.now().toString())
            : ((calEvent.schedule_data?.eventId as string | undefined) || calEvent.id || Date.now().toString());
          ...
          const transformedEvent: Event = {
            ...
            recordStreaming: (calEvent.schedule_data?.recordStreaming as string) || 'None',
            isQuickMode: isQm,
  ...
  const addEvent = async () => {
    ...
    try {
      ...
      console.log('✅ Calendar event saved to LAN DB:', savedCalendarEvent);
      if (savedCalendarEvent?.id) {
        setEvents((prev) =>
          prev.map((e) =>
            e.id === newEvent.id ? { ...e, calendarId: savedCalendarEvent.id } : e
          )
        );
      }
      ...
      const runOfShowData = await DatabaseService.saveRunOfShowData({
        ...
        userName: getOfflineDisplayName(),
        userRole: 'EDITOR',
      });
      ...
      try {
        ...
        if (existingData) {
          ...
          await apiClient.saveRunOfShowData({
            ...
            last_modified_by: getOfflineUserId(),
            last_modified_by_name: getOfflineDisplayName(),
            last_modified_by_role: 'EDITOR',
          });
  ...
  const handleRoleSelected = async (role: string) => {
    ...
    try {
      ...
      console.error('❌ Failed to save offline role:', error);
    }
