...
const FullScreenTimer: React.FC<FullScreenTimerProps> = ({
  ...
  useEffect(() => {
    if (secondaryTimer) {
      console.log('🔍 Secondary timer prop updated:', {
        secondaryTimer,
        supabaseOnly,
        hasSecondaryTimer: !!secondaryTimer
      });
    }
  }, [secondaryTimer, supabaseOnly]);
  ...
  useEffect(() => {
    ...
    // Poll every 3 seconds when in hybrid mode
    const interval = setInterval(fetchHybridData, 3000);
        ...
        <div className="text-white flex items-center justify-end gap-3 whitespace-nowrap">
          <span>
            {(() => {
              ...
              return mainTimer ? (mainTimer.cue.includes('CUE') ? mainTimer.cue.replace(/CUE(\d+)/, 'CUE $1') : `CUE ${mainTimer.cue}`) : 'No CUE';
            })()}
          </span>
