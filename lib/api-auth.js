  ...
  try {
    ...
    if (isUndefinedColumnError(err, 'is_creative')) {
      try {
        ...
        return withDefaults(r.rows[0] || null, { is_creative: false });
      } catch (errCreative) {
    ...
    if (isUndefinedColumnError(err, 'is_comms')) {
      try {
        ...
        return withDefaults(r.rows[0] || null, { is_comms: false });
      } catch (errComms) {
    ...
    if (isUndefinedColumnError(err, 'is_bts_crew') || isUndefinedColumnError(err, 'is_comms')) {
      try {
        ...
        return withDefaults(r.rows[0] || null, { is_bts_crew: false, is_comms: false, is_creative: false });
      } catch (errBts) {
    ...
    if (isUndefinedColumnError(err, 'is_catering') || isUndefinedColumnError(err, 'is_bts_crew') || isUndefinedColumnError(err, 'is_comms')) {
      try {
        ...
        return withDefaults(r.rows[0] || null, { is_catering: false, is_bts_crew: false, is_comms: false, is_creative: false });
      } catch (err2) {
...
function userCanManageAccess(auth) {
  if (!auth) return false;
  if (auth.isAdmin) return true;
  // BTS Crew matches Event Manager platform capabilities (distinct label).
  return !!auth.isEventManager || !!auth.isBtsCrew;
}
...
function resolveEventManagerFlag(access, isAdmin) {
  ...
  return access.is_event_manager === true;
}
  ...
  app.get('/api/admin/access-requests', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        if (isUndefinedColumnError(joinErr, 'is_creative')) {
        ...
        r = await client.query(
          `UPDATE public.api_user_access
           SET status = 'approved',
               ...
               is_bts_crew = $6,
               is_creative = $7,
               reviewed_at = NOW(), reviewed_by = $8
           WHERE id = $1
           RETURNING *`,
          [id, makeAdmin, isEventManager, isComms, isCatering, isCrew, isCreative, reviewedBy]
        );
      } catch (roleErr) {
        // Older DBs without newer role columns — fall back to admin-only approve.
        if (isUndefinedColumnError(roleErr, 'is_creative') || isUndefinedColumnError(roleErr, 'is_comms') || isUndefinedColumnError(roleErr, 'is_bts_crew') || isUndefinedColumnError(roleErr, 'is_catering') || isUndefinedColumnError(roleErr, 'is_event_manager')) {
          r = await client.query(
            `UPDATE public.api_user_access
  ...
  app.patch('/api/admin/access-requests/:id', async (req, res) => {
    ...
    try {
      ...
      if (body.is_admin !== undefined) {
        ...
        if (parseBoolBody(body.is_admin)) {
          updates.push('is_event_manager = FALSE');
          updates.push('is_catering = FALSE');
          updates.push('is_bts_crew = FALSE');
          updates.push('is_comms = FALSE');
          updates.push('is_creative = FALSE');
        }
      }

      if (body.is_event_manager !== undefined) {
        updates.push(`is_event_manager = $${param++}`);
        values.push(parseBoolBody(body.is_event_manager));
        if (parseBoolBody(body.is_event_manager)) {
          updates.push('is_admin = FALSE');
          updates.push('is_catering = FALSE');
          updates.push('is_bts_crew = FALSE');
          updates.push('is_comms = FALSE');
          updates.push('is_creative = FALSE');
      ...
      if (body.is_catering !== undefined) {
        updates.push(`is_catering = $${param++}`);
        values.push(parseBoolBody(body.is_catering));
        if (parseBoolBody(body.is_catering)) {
          updates.push('is_admin = FALSE');
          updates.push('is_event_manager = FALSE');
          updates.push('is_bts_crew = FALSE');
          updates.push('is_comms = FALSE');
      ...
      if (body.is_bts_crew !== undefined) {
        updates.push(`is_bts_crew = $${param++}`);
        values.push(parseBoolBody(body.is_bts_crew));
        if (parseBoolBody(body.is_bts_crew)) {
          updates.push('is_admin = FALSE');
          updates.push('is_event_manager = FALSE');
          updates.push('is_catering = FALSE');
          updates.push('is_comms = FALSE');
      ...
      if (body.is_comms !== undefined) {
        updates.push(`is_comms = $${param++}`);
        values.push(parseBoolBody(body.is_comms));
        if (parseBoolBody(body.is_comms)) {
          updates.push('is_admin = FALSE');
          updates.push('is_event_manager = FALSE');
          updates.push('is_bts_crew = FALSE');
          updates.push('is_catering = FALSE');
      ...
      if (body.is_creative !== undefined) {
        updates.push(`is_creative = $${param++}`);
        values.push(parseBoolBody(body.is_creative));
        if (parseBoolBody(body.is_creative)) {
          updates.push('is_admin = FALSE');
          updates.push('is_event_manager = FALSE');
          updates.push('is_bts_crew = FALSE');
          updates.push('is_catering = FALSE');
