...
async function assignPortalToken(pool, accessId) {
  ...
  const tokenHash = hashPortalToken(rawToken);
  await pool.query(`UPDATE public.api_user_access SET portal_token_hash = $1 WHERE id = $2`, [
    tokenHash,
    accessId,
  ]);
  return rawToken;
...
function registerAuthRoutes(app, pool, options = {}) {
  ...
  app.post('/api/auth/request-access', async (req, res) => {
    try {
      ...
      const r = await pool.query(
        `INSERT INTO public.api_user_access (neon_user_id, email, full_name, status, is_admin, portal_token_hash)
         VALUES (NULL, $1, $2, $3, $4, $5)
         RETURNING id, email, full_name, status, is_admin, requested_at`,
        [normalizedEmail, name, status, isFirstAdmin, portalTokenHash]
      );
      const row = r.rows[0];
      const portalUrl = buildAccessPortalUrl(origin, portalToken);
  ...
  app.post('/api/admin/access-requests/:id/approve', async (req, res) => {
    ...
    try {
      ...
      const portalToken = await assignPortalToken(pool, request.id);
