...
const { passwordPolicyError, PASSWORD_REQUIREMENTS_TEXT } = require('./password-policy');
const { queueLoginSecurityAdminAlerts } = require('./login-security-alerts');
const {
  recordUnauthorizedApiAccess,
  recordIntegrationForbidden,
  isUnauthorizedQuarantined,
  quarantineRetryAfterSec,
} = require('./ops-alerts');
const {
  hashPortalToken,
  generatePortalToken,
...
const PUBLIC_GRAPHICS_PATHS = new Set([
  ...
  '/api/schedule.xml',
  '/api/schedule.csv',
  '/api/custom-columns.xml',
  '/api/custom-columns.csv',
...
function extractEventIdFromRequest(req) {
  ...
  // Middleware often runs before route params are populated — parse from path.
  const pathname = getApiPathname(req);
  const match = pathname.match(
    /\/api\/(?:active-timers|run-of-show-data|completed-cues|overtime-minutes|show-start-overtime|timer-messages|indented-cues|sub-cue-timers|show-mode)\/([0-9a-f-]{36})(?:\/|$)/i
  );
...
function pathRequiresAuth(pathname, method, config) {
  // Public GET: display sync flag (stale OBS/tabs can learn to stop without a session)
  if (isReadMethod(method) && /^\/api\/display-sync\/[0-9a-f-]{36}$/i.test(pathname)) return false;
  ...
  // Public guest view payload (token in path) — GET only
  if (isReadMethod(method) && pathname.startsWith('/api/guest-event/')) return false;
  // Public Quick Mode operator session bootstrap (token in path) — GET only
  if (isReadMethod(method) && pathname.startsWith('/api/quick-mode-operator/')) return false;
  // Public Stream Request form (token in path)
  if (isReadMethod(method) && pathname.startsWith('/api/stream-request/')) return false;
  if (method === 'POST' && pathname.startsWith('/api/stream-request/')) return false;
  // Public Stream Guide TV listings (Streaming / Stream+Rec events only)
  if (pathname === '/api/stream-guide/summary' && isReadMethod(method)) return false;
  // Public training booking (list slots + book + download .ics)
  if (pathname === '/api/training/slots' && isReadMethod(method)) return false;
  if (pathname === '/api/training/book' && method === 'POST') return false;
  if (isReadMethod(method) && /^\/api\/training\/booking\/[^/]+\/ics$/.test(pathname)) return false;
...
function checkIntegrationAccess(auth, req) {
  ...
  if (method === 'DELETE' || method === 'POST' || method === 'PUT' || method === 'PATCH') {
    ...
    // Companion timer/cue control (+1/-1 duration, start/stop, load cue, sub-cues).
    // Note: /api/active-timers/.../duration is control — not schedule "write".
    if (
      path.includes('/timers/') ||
      path.includes('/active-timers/') ||
      path.includes('/cues/') ||
      path.includes('/sub-cue-timers')
    ) {
...
async function getAccessRecord(pool, neonUserId, email) {
  ...
  const columnsWithProducer =
    'id, neon_user_id, email, full_name, status, is_admin, is_event_manager, is_producer, is_catering, is_bts_crew, is_comms, is_creative, dashboard_enabled, requested_at, reviewed_at, reviewed_by, notes, password_set_at';
  const columnsWithCreative =
    'id, neon_user_id, email, full_name, status, is_admin, is_event_manager, is_catering, is_bts_crew, is_comms, is_creative, dashboard_enabled, requested_at, reviewed_at, reviewed_by, notes, password_set_at';
  const columnsWithComms =
    'id, neon_user_id, email, full_name, status, is_admin, is_event_manager, is_catering, is_bts_crew, is_comms, dashboard_enabled, requested_at, reviewed_at, reviewed_by, notes, password_set_at';
  const columnsWithBtsCrew =
    'id, neon_user_id, email, full_name, status, is_admin, is_event_manager, is_catering, is_bts_crew, dashboard_enabled, requested_at, reviewed_at, reviewed_by, notes, password_set_at';
  const columnsWithCatering =
    'id, neon_user_id, email, full_name, status, is_admin, is_event_manager, is_catering, dashboard_enabled, requested_at, reviewed_at, reviewed_by, notes, password_set_at';
  const columnsWithManager =
    'id, neon_user_id, email, full_name, status, is_admin, is_event_manager, dashboard_enabled, requested_at, reviewed_at, reviewed_by, notes, password_set_at';
  const columnsLegacy =
    'id, neon_user_id, email, full_name, status, is_admin, dashboard_enabled, requested_at, reviewed_at, reviewed_by, notes, password_set_at';
  ...
  const withDefaults = (row, extras = {}) => {
    if (!row) return null;
    return {
      ...row,
      is_event_manager: row.is_event_manager === true,
      is_producer: row.is_producer === true,
      is_catering: row.is_catering === true,
      is_bts_crew: row.is_bts_crew === true,
      is_comms: row.is_comms === true,
  ...
  try {
    const r = await runSelect(columnsWithProducer);
    return withDefaults(r.rows[0] || null);
  } catch (err) {
    if (err.code === '42P01') return null;
    if (isUndefinedColumnError(err, 'is_producer')) {
      try {
        const r = await runSelect(columnsWithCreative);
        return withDefaults(r.rows[0] || null, { is_producer: false });
      } catch (errProducer) {
        if (!isUndefinedColumnError(errProducer, 'is_creative')) {
          if (errProducer.code === '42P01') return null;
          throw errProducer;
        }
        err = errProducer;
      }
    }
    if (isUndefinedColumnError(err, 'is_creative')) {
      try {
        const r = await runSelect(columnsWithComms);
        return withDefaults(r.rows[0] || null, { is_creative: false, is_producer: false });
      } catch (errCreative) {
        if (!isUndefinedColumnError(errCreative, 'is_comms')) {
          if (errCreative.code === '42P01') return null;
          throw errCreative;
        }
      }
    }
    if (isUndefinedColumnError(err, 'is_comms')) {
      try {
        const r = await runSelect(columnsWithBtsCrew);
        return withDefaults(r.rows[0] || null, { is_comms: false, is_producer: false });
      } catch (errComms) {
        if (!isUndefinedColumnError(errComms, 'is_bts_crew')) {
          if (errComms.code === '42P01') return null;
          throw errComms;
        }
      }
    }
    if (isUndefinedColumnError(err, 'is_bts_crew') || isUndefinedColumnError(err, 'is_comms')) {
      try {
        const r = await runSelect(columnsWithCatering);
        return withDefaults(r.rows[0] || null, {
          is_bts_crew: false,
          is_comms: false,
          is_creative: false,
        ...
        if (!isUndefinedColumnError(errBts, 'is_catering')) {
          if (errBts.code === '42P01') return null;
          throw errBts;
        }
      }
    }
    if (isUndefinedColumnError(err, 'is_catering') || isUndefinedColumnError(err, 'is_bts_crew') || isUndefinedColumnError(err, 'is_comms')) {
      try {
        const r = await runSelect(columnsWithManager);
        return withDefaults(r.rows[0] || null, {
          is_catering: false,
          is_bts_crew: false,
          is_comms: false,
        ...
        if (!isUndefinedColumnError(err2, 'is_event_manager')) {
          if (err2.code === '42P01') return null;
          throw err2;
        }
        const r = await runSelect(columnsLegacy);
        return withDefaults(r.rows[0] || null, {
...
/** Replace allowlist. Empty array = unrestricted (all events). */
async function saveUserEventAccess(client, accessId, eventIds) {
  const uniqueIds = [
    ...new Set((eventIds || []).map((value) => String(value).trim()).filter(Boolean)),
  ];
  await client.query(`DELETE FROM public.api_user_event_access WHERE access_id = $1`, [accessId]);
...
/**
 * If the creator has a selected-events allowlist, append the new event so they
 * can see it going forward. Unrestricted users (null allowlist) are unchanged.
 */
async function grantCreatedEventToRestrictedUser(pool, auth, eventId) {
  if (!pool || !auth || !eventId) return false;
  if (auth.isAdmin) return false;
  if (!auth.accessId) return false;
  // null = unrestricted (all events); only auto-grant when already restricted
  if (!auth.allowedEventIds) return false;
...
function userCanManageAccess(auth) {
  if (!auth) return false;
  if (auth.isAdmin) return true;
  // BTS Crew / Producer match Event Manager platform capabilities (distinct labels).
  return !!auth.isEventManager || !!auth.isProducer || !!auth.isBtsCrew;
}

function userCanManageSpeakers(auth) {
  if (!auth) return false;
  if (auth.isAdmin) return true;
  return !!auth.isProducer;
}

function userCanAccessPreflightChecklist(auth) {
  if (!auth) return false;
  if (auth.isAdmin) return true;
  return !!auth.isBtsCrew;
}

function authorizeAccessManagerRoute(req, res, requireAdminAuth) {
  if (userCanManageAccess(req.auth)) return true;
  if (requireAdminAuth && requireAdminAuth(req, res)) return true;
  if (res.headersSent) return false;
  res.status(403).json({
    error: 'Forbidden',
    message: 'Administrator, event manager, or BTS crew access required.',
  });
  return false;
}

function authorizeAdminOnlyRoute(req, res, requireAdminAuth) {
...
function resolveEventManagerFlag(access, isAdmin) {
  if (!access || isAdmin) return false;
  // Producers also get EM-equivalent rights even if only is_producer is set.
  return access.is_event_manager === true || access.is_producer === true;
}

function resolveCateringFlag(access, isAdmin) {
  if (!access || isAdmin) return false;
  return access.is_catering === true;
}

function resolveBtsCrewFlag(access, isAdmin) {
  if (!access || isAdmin) return false;
  return access.is_bts_crew === true;
}

function resolveCommsFlag(access, isAdmin) {
  if (!access || isAdmin) return false;
  return access.is_comms === true;
}

function resolveCreativeFlag(access, isAdmin) {
  if (!access || isAdmin) return false;
  return access.is_creative === true;
}

function resolveProducerFlag(access, isAdmin) {
  if (!access || isAdmin) return false;
  return access.is_producer === true;
}
...
async function buildNeonUserAuth(pool, access, identity) {
  ...
  const isEventManager = approved && resolveEventManagerFlag(access, isAdmin);
  const isCatering = approved && resolveCateringFlag(access, isAdmin);
  const isBtsCrew = approved && resolveBtsCrewFlag(access, isAdmin);
  const isComms = approved && resolveCommsFlag(access, isAdmin);
  const isCreative = approved && resolveCreativeFlag(access, isAdmin);
  const isProducer = approved && resolveProducerFlag(access, isAdmin);
...
async function resolveAuth(pool, rawToken) {
  ...
  // Stable public Quick Mode operator token (ros_qmop_…) — event-scoped control until expires_at
  if (rawToken.startsWith('ros_qmop_')) {
    try {
      const {
        isQmOpToken,
        lookupQmOpByToken,
        touchQmOpLinkUsed,
      } = require('./quick-mode-operator-links');
      if (!isQmOpToken(rawToken)) return null;
      const row = await lookupQmOpByToken(pool, rawToken);
      if (!row) return null;
      if (!row.expires_at || new Date(row.expires_at) <= new Date()) return null;
      touchQmOpLinkUsed(pool, row.id);
      return {
        type: 'quick_mode_operator',
        tokenId: row.id,
...
async function createNeonApiSession(pool, config, identity, fullName) {
  ...
  return {
    ...
    is_event_manager: resolveEventManagerFlag(accessRow, !!accessRow.is_admin),
    is_catering: resolveCateringFlag(accessRow, !!accessRow.is_admin),
    is_bts_crew: resolveBtsCrewFlag(accessRow, !!accessRow.is_admin),
    is_comms: resolveCommsFlag(accessRow, !!accessRow.is_admin),
    is_creative: resolveCreativeFlag(accessRow, !!accessRow.is_admin),
    is_producer: resolveProducerFlag(accessRow, !!accessRow.is_admin),
...
function createApiAuthMiddleware(pool, config) {
  return async function apiAuthMiddleware(req, res, next) {
    ...
    // Cut off stale OBS/Edge spam before token lookup once quarantined
    if (
      pathRequiresAuth(pathname, req.method, config) &&
      isUnauthorizedQuarantined(req, pathname)
    ) {
      const retryAfter = quarantineRetryAfterSec(req, pathname);
      res.setHeader('Retry-After', String(retryAfter));
      return res.status(429).json({
        error: 'Too many unauthorized requests',
        stopPolling: true,
        message:
          'This client was temporarily blocked after repeated expired/missing sessions. Close or refresh the tab (or pause Displays for the event).',
    ...
    if (req.auth.type === 'quick_mode_operator') {
      const { qmOpAccessAllowed } = require('./quick-mode-operator-links');
      const requestEventId = extractEventIdFromRequest(req);
      if (qmOpAccessAllowed(req.auth, pathname, req.method, requestEventId)) {
        return next();
      }
      return res.status(403).json({
        error: 'Forbidden',
        message: 'Quick Mode operator link is not allowed for this action or event.',
      });
    }
...
function registerAuthRoutes(app, pool, options = {}) {
  ...
  app.post('/api/auth/complete-account-setup', async (req, res) => {
    try {
      ...
      if (passwordError) {
        return res.status(400).json({ error: passwordError, hint: PASSWORD_REQUIREMENTS_TEXT });
      }
  ...
  app.get('/api/auth/access-status', async (req, res) => {
    try {
      ...
      return res.json({
        ...
        is_event_manager: auth.isEventManager,
        is_catering: auth.isCatering,
        is_bts_crew: auth.isBtsCrew,
        is_comms: auth.isComms,
        is_creative: auth.isCreative,
        is_producer: auth.isProducer,
  ...
  async function mintIntegrationToken(pool, { tokenName, scopeList, eventId, expiresAt, createdBy }) {
    ...
    return { rawToken, record: r.rows[0] };
  }
  ...
  app.post('/api/admin/integration-tokens', async (req, res) => {
    ...
    try {
      ...
      const createdBy = req.auth?.type === 'user' && req.auth.userId ? req.auth.userId : null;
      const { rawToken, record } = await mintIntegrationToken(pool, {
        tokenName,
        scopeList,
      ...
      res.status(201).json({
        token: rawToken,
        record: { ...record, token: rawToken },
        message: 'Copy this token now — it will not be shown again. Use Regenerate if you lose it.',
      });
    } catch (err) {
      if (err.code === '42P01') {
        return res.status(503).json({ error: 'Auth tables not found. Run migration 026 on Neon.' });
  ...
  app.post('/api/admin/integration-tokens/:id/regenerate', async (req, res) => {
    if (!req.auth?.isAdmin && (!requireAdminAuth || !requireAdminAuth(req, res))) return;
    try {
      const { id } = req.params;
      const existing = await pool.query(
        `SELECT id, name, scopes, event_id, expires_at
         FROM public.api_integration_tokens
         WHERE id = $1`,
        [id]
      );
      ...
      await pool.query('DELETE FROM public.api_integration_tokens WHERE id = $1', [id]);
      ...
      const { rawToken, record } = await mintIntegrationToken(pool, {
        tokenName: row.name,
        scopeList: row.scopes || ['read', 'control'],
      ...
      res.status(201).json({
        ...
        record: { ...record, token: rawToken },
        message:
          'New token created — copy it now. It will not be shown again. Update Companion / Spout with the new secret.',
      });
    } catch (err) {
      console.error('[integration-tokens regenerate]', err);
      res.status(500).json({ error: err.message });
    }
  });

  app.delete('/api/admin/integration-tokens/:id/permanent', async (req, res) => {
    if (!req.auth?.isAdmin && (!requireAdminAuth || !requireAdminAuth(req, res))) return;
    try {
      const { id } = req.params;
      const r = await pool.query(
        `DELETE FROM public.api_integration_tokens
      ...
      if (!r.rows.length) {
        return res.status(400).json({
          error: 'Token not found or still active. Revoke it first, then delete.',
        });
      }
      res.json({ ok: true });
    } catch (err) {
      console.error('[integration-tokens permanent delete]', err);
      res.status(500).json({ error: err.message });
    }
  });
  ...
  app.get('/api/admin/access-requests', async (req, res) => {
    if (!authorizeAccessManagerRoute(req, res, requireAdminAuth)) return;
    try {
      ...
      let cateringNeedsMigration = false;
      let btsCrewNeedsMigration = false;
      let commsNeedsMigration = false;
      let creativeNeedsMigration = false;
      let producerNeedsMigration = false;
      let r;
      const selectWithProducer = (statusFilter) => {
        const eventJoin = `
                 LEFT JOIN (
        ...
        const baseSelect = `SELECT a.id, a.neon_user_id, a.email, a.full_name, a.status, a.is_admin, a.is_event_manager, a.is_producer, a.is_catering, a.is_bts_crew, a.is_comms, a.is_creative, a.dashboard_enabled, a.requested_at, a.reviewed_at, a.reviewed_by, a.notes, a.password_set_at, a.portal_token,
      ...
      const selectWithCreative = (statusFilter) => {
        const eventJoin = `
                 LEFT JOIN (
                   SELECT access_id, COUNT(*)::int AS n
                   FROM public.api_user_event_access
        ...
        const baseSelect = `SELECT a.id, a.neon_user_id, a.email, a.full_name, a.status, a.is_admin, a.is_event_manager, a.is_catering, a.is_bts_crew, a.is_comms, a.is_creative, a.dashboard_enabled, a.requested_at, a.reviewed_at, a.reviewed_by, a.notes, a.password_set_at, a.portal_token,
                        COALESCE(ea_counts.n, 0)::int AS event_access_count
                 FROM public.api_user_access a${eventJoin}`;
        if (statusFilter == null) {
          return pool.query(`${baseSelect} ORDER BY a.requested_at DESC`);
      ...
      const selectWithComms = (statusFilter) => {
        const eventJoin = `
                 LEFT JOIN (
                   SELECT access_id, COUNT(*)::int AS n
                   FROM public.api_user_event_access
        ...
        const baseSelect = `SELECT a.id, a.neon_user_id, a.email, a.full_name, a.status, a.is_admin, a.is_event_manager, a.is_catering, a.is_bts_crew, a.is_comms, a.dashboard_enabled, a.requested_at, a.reviewed_at, a.reviewed_by, a.notes, a.password_set_at, a.portal_token,
                        COALESCE(ea_counts.n, 0)::int AS event_access_count
                 FROM public.api_user_access a${eventJoin}`;
        if (statusFilter == null) {
          return pool.query(`${baseSelect} ORDER BY a.requested_at DESC`);
      ...
      const selectWithBtsCrew = (statusFilter) => {
        const eventJoin = `
                 LEFT JOIN (
                   SELECT access_id, COUNT(*)::int AS n
                   FROM public.api_user_event_access
        ...
        const baseSelect = `SELECT a.id, a.neon_user_id, a.email, a.full_name, a.status, a.is_admin, a.is_event_manager, a.is_catering, a.is_bts_crew, a.dashboard_enabled, a.requested_at, a.reviewed_at, a.reviewed_by, a.notes, a.password_set_at, a.portal_token,
                        COALESCE(ea_counts.n, 0)::int AS event_access_count
                 FROM public.api_user_access a${eventJoin}`;
        if (statusFilter == null) {
          return pool.query(`${baseSelect} ORDER BY a.requested_at DESC`);
      ...
      const selectWithCatering = (statusFilter) => {
        const eventJoin = `
                 LEFT JOIN (
                   SELECT access_id, COUNT(*)::int AS n
                   FROM public.api_user_event_access
        ...
        const baseSelect = `SELECT a.id, a.neon_user_id, a.email, a.full_name, a.status, a.is_admin, a.is_event_manager, a.is_catering, a.dashboard_enabled, a.requested_at, a.reviewed_at, a.reviewed_by, a.notes, a.password_set_at, a.portal_token,
                        COALESCE(ea_counts.n, 0)::int AS event_access_count
                 FROM public.api_user_access a${eventJoin}`;
        if (statusFilter == null) {
          return pool.query(`${baseSelect} ORDER BY a.requested_at DESC`);
      ...
      const selectWithDashboard = (statusFilter) => {
        ...
        const baseSelect = `SELECT a.id, a.neon_user_id, a.email, a.full_name, a.status, a.is_admin, a.is_event_manager, a.dashboard_enabled, a.requested_at, a.reviewed_at, a.reviewed_by, a.notes, a.password_set_at, a.portal_token,
                        COALESCE(ea_counts.n, 0)::int AS event_access_count
                 FROM public.api_user_access a${eventJoin}`;
        if (statusFilter == null) {
          return pool.query(`${baseSelect} ORDER BY a.requested_at DESC`);
      ...
      try {
        r = await selectWithProducer(filter === 'all' ? null : filter);
      } catch (joinErr) {
        if (isUndefinedColumnError(joinErr, 'is_producer')) {
          producerNeedsMigration = true;
          try {
            r = await selectWithCreative(filter === 'all' ? null : filter);
          } catch (producerErr) {
            joinErr = producerErr;
          }
        }
        if (!r && isUndefinedColumnError(joinErr, 'is_creative')) {
          creativeNeedsMigration = true;
          try {
            r = await selectWithComms(filter === 'all' ? null : filter);
          } catch (creativeErr) {
            joinErr = creativeErr;
          }
        }
        if (!r && isUndefinedColumnError(joinErr, 'is_comms')) {
          commsNeedsMigration = true;
          try {
            r = await selectWithBtsCrew(filter === 'all' ? null : filter);
          } catch (btsErr) {
            joinErr = btsErr;
          }
        }
        if (!r && isUndefinedColumnError(joinErr, 'is_bts_crew')) {
          btsCrewNeedsMigration = true;
          try {
            r = await selectWithCatering(filter === 'all' ? null : filter);
          } catch (cateringErr) {
            if (isUndefinedColumnError(cateringErr, 'is_catering')) {
              cateringNeedsMigration = true;
              try {
                r = await selectWithDashboard(filter === 'all' ? null : filter);
              } catch (joinErr2) {
                if (isUndefinedColumnError(joinErr2, 'is_event_manager')) {
                  r = await selectWithoutDashboard(filter === 'all' ? null : filter);
                } else if (isUndefinedColumnError(joinErr2, 'dashboard_enabled')) {
                  dashboardNeedsMigration = true;
                  r = await selectWithoutDashboard(filter === 'all' ? null : filter);
                } else if (joinErr2.code === '42P01') {
            ...
            } else if (isUndefinedColumnError(cateringErr, 'is_event_manager')) {
              r = await selectWithoutDashboard(filter === 'all' ? null : filter);
            } else if (isUndefinedColumnError(cateringErr, 'dashboard_enabled')) {
              dashboardNeedsMigration = true;
              r = await selectWithoutDashboard(filter === 'all' ? null : filter);
            } else if (cateringErr.code === '42P01') {
              eventAccessNeedsMigration = true;
              r = await selectWithoutDashboard(filter === 'all' ? null : filter);
            } else {
              throw cateringErr;
        ...
        } else if (isUndefinedColumnError(joinErr, 'is_catering')) {
          cateringNeedsMigration = true;
          try {
            r = await selectWithDashboard(filter === 'all' ? null : filter);
          } catch (joinErr2) {
        ...
        } else if (isUndefinedColumnError(joinErr, 'is_event_manager')) {
          r = await selectWithoutDashboard(filter === 'all' ? null : filter);
        } else if (isUndefinedColumnError(joinErr, 'dashboard_enabled')) {
          dashboardNeedsMigration = true;
          r = await selectWithoutDashboard(filter === 'all' ? null : filter);
      ...
      res.json({
        requests,
        eventAccessNeedsMigration,
        dashboardNeedsMigration,
        cateringNeedsMigration,
        btsCrewNeedsMigration,
        commsNeedsMigration,
        creativeNeedsMigration,
        producerNeedsMigration,
      });
    } catch (err) {
  ...
  app.post('/api/admin/access-requests/:id/approve', async (req, res) => {
    ...
    try {
      const { id } = req.params;
      const body = req.body || {};
      const isFullAdmin = req.auth?.isAdmin === true;
      const reviewedBy = req.auth?.email || req.auth?.userId || 'admin';
      const hasEventIds = Object.prototype.hasOwnProperty.call(body, 'event_ids');
      const eventIds = hasEventIds && Array.isArray(body.event_ids) ? body.event_ids : null;

      // Only full admins may assign platform roles at approve time.
      let role = 'user';
      if (isFullAdmin) {
        const rawRole = String(body.role || '').toLowerCase().trim();
        if (body.make_admin || rawRole === 'admin') role = 'admin';
        else if (rawRole === 'producer') role = 'producer';
        else if (rawRole === 'event_manager' || rawRole === 'event-manager') role = 'event_manager';
        else if (rawRole === 'comms') role = 'comms';
        else if (rawRole === 'catering') role = 'catering';
        else if (rawRole === 'creative') role = 'creative';
        else if (rawRole === 'crew' || rawRole === 'bts_crew' || rawRole === 'bts-crew') role = 'crew';
        else role = 'user';
      }
      const makeAdmin = role === 'admin';
      const isProducer = role === 'producer';
      const isEventManager = role === 'event_manager' || isProducer;
      const isComms = role === 'comms';
      const isCatering = role === 'catering';
        ...
        r = await client.query(
          `UPDATE public.api_user_access
           SET status = 'approved',
               ...
               is_bts_crew = $7,
               is_creative = $8,
               reviewed_at = NOW(), reviewed_by = $9
           WHERE id = $1
           RETURNING *`,
          [id, makeAdmin, isEventManager, isProducer, isComms, isCatering, isCrew, isCreative, reviewedBy]
        );
      } catch (roleErr) {
        // Older DBs without newer role columns — fall back to admin-only approve.
        if (
          isUndefinedColumnError(roleErr, 'is_producer') ||
          isUndefinedColumnError(roleErr, 'is_creative') ||
          isUndefinedColumnError(roleErr, 'is_comms') ||
          isUndefinedColumnError(roleErr, 'is_bts_crew') ||
          isUndefinedColumnError(roleErr, 'is_catering') ||
          isUndefinedColumnError(roleErr, 'is_event_manager')
        ) {
          r = await client.query(
            `UPDATE public.api_user_access
      ...
      let savedEventIds = null;
      if (!makeAdmin && !request.is_admin && eventIds !== null) {
        savedEventIds = await saveUserEventAccess(client, id, eventIds);
      }
      ...
      res.json({
        ...
        unrestricted: savedEventIds == null || savedEventIds.length === 0,
      });
    } catch (err) {
  ...
  app.post('/api/admin/access-requests/:id/reject', async (req, res) => {
    ...
    try {
      ...
      if (existingRes.rows.length === 0) return res.status(404).json({ error: 'Request not found.' });
      const existing = existingRes.rows[0];

      // Event managers may only reject pending requests — not remove approved users.
      if (!isFullAdmin && existing.status !== 'pending') {
        return res.status(403).json({
          error: 'Event managers can only reject pending access requests. Ask an administrator to change approved users.',
        });
      }
    ...
    try {
      ...
      if (!isFullAdmin) {
        if (
          ...
          body.dashboard_enabled !== undefined ||
          body.is_event_manager !== undefined ||
          body.is_catering !== undefined ||
          body.is_bts_crew !== undefined ||
          body.is_comms !== undefined ||
          body.is_creative !== undefined ||
          body.is_producer !== undefined
        ) {
          return res.status(403).json({ error: 'Only administrators can change roles or dashboard access.' });
        }
        if (body.reset_account) {
          return res.status(403).json({ error: 'Only administrators can reset accounts.' });
        }
      }
      ...
      if (body.status !== undefined) {
        ...
        // Event managers cannot demote / remove already-approved users.
        if (!isFullAdmin && existing.status === 'approved' && status !== 'approved') {
          return res.status(403).json({
            error: 'Event managers cannot remove or change approved users. Ask an administrator.',
          });
        }
      ...
      const roleColumnPatches = new Map();

      const setRoleColumn = (column, value) => {
        roleColumnPatches.set(column, value);
      };

      for (const column of ROLE_COLUMNS) {
        const bodyKey = column;
        if (body[bodyKey] === undefined) continue;
        const enabled = parseBoolBody(body[bodyKey]);
        setRoleColumn(column, enabled);
        if (enabled) {
          for (const other of ROLE_COLUMNS) {
            if (other !== column) setRoleColumn(other, false);
          }
        }
      }

      // Producer always carries EM rights (and must win over mutual-exclusion with is_event_manager).
      if (body.is_producer !== undefined && parseBoolBody(body.is_producer)) {
        roleColumnPatches.set('is_producer', true);
        roleColumnPatches.set('is_event_manager', true);
        roleColumnPatches.set('is_admin', false);
        roleColumnPatches.set('is_catering', false);
        roleColumnPatches.set('is_bts_crew', false);
        roleColumnPatches.set('is_comms', false);
        roleColumnPatches.set('is_creative', false);
      }

      for (const [column, value] of roleColumnPatches) {
      ...
      try {
        ...
        if (isUndefinedColumnError(updateErr, 'is_producer')) {
          return res.status(503).json({
            error: 'Run migration 061_add_producer_role.sql on Neon to enable Producer roles.',
          });
        }
        if (isUndefinedColumnError(updateErr, 'is_creative')) {
          return res.status(503).json({
            error: 'Run migration 056_add_creative_role.sql on Neon to enable Creative roles.',
          });
        }
        if (isUndefinedColumnError(updateErr, 'is_comms')) {
          return res.status(503).json({
            error: 'Run migration 048_add_comms_role.sql on Neon to enable Comms roles.',
          });
        }
        if (isUndefinedColumnError(updateErr, 'is_bts_crew')) {
          return res.status(503).json({
            error: 'Run migration 044_add_bts_crew_role.sql on Neon to enable BTS crew roles.',
          });
        }
        if (isUndefinedColumnError(updateErr, 'is_catering')) {
          return res.status(503).json({
            error: 'Run migration 039_add_catering_role.sql on Neon to enable catering roles.',
          });
        }
        if (isUndefinedColumnError(updateErr, 'is_event_manager')) {
          return res.status(503).json({
            error: 'Run migration 033_add_event_manager_role.sql on Neon to enable event manager roles.',
          });
        }
  ...
  app.delete('/api/admin/access-requests/:id', async (req, res) => {
    if (!authorizeAdminOnlyRoute(req, res, requireAdminAuth)) return;
    try {
      const { id } = req.params;
    ...
    try {
      ...
      try {
        ...
        if (isUndefinedColumnError(err, 'deleted_at')) {
          eventsRes = await pool.query(
            `SELECT id, name, date, schedule_data FROM calendar_events ORDER BY date DESC`
          );
        } else if (isUndefinedColumnError(err, 'schedule_data')) {
          eventsRes = await pool.query(`SELECT id, name, date FROM calendar_events ORDER BY date DESC`);
        } else {
          throw err;
        }
      }
      const events = (eventsRes.rows || []).map((row) => {
        ...
        if (!scheduleData || typeof scheduleData !== 'object') scheduleData = {};
        const name = row.name;
        const isQuickMode =
          scheduleData.quickMode === true ||
          scheduleData.source === 'quick-mode' ||
          /^Quick Mode/i.test(String(name || ''));
        return {
          id: String(row.id),
          name,
          date:
  ...
  app.put('/api/admin/access-requests/:id/event-access', async (req, res) => {
    ...
    try {
      ...
      const accessRes = await client.query(
        `SELECT id, is_admin FROM public.api_user_access WHERE id = $1`,
        [id]
      );
      if (accessRes.rows.length === 0) return res.status(404).json({ error: 'User not found.' });
      if (accessRes.rows[0].is_admin) {
        return res.status(400).json({ error: 'Administrators always have access to all events.' });
      }

      await client.query('BEGIN');
      const uniqueIds = await saveUserEventAccess(client, id, body.event_ids);
      await client.query('COMMIT');
...
module.exports = {
  ...
  userCanManageAccess,
  userCanManageSpeakers,
  userCanAccessPreflightChecklist,
  filterCalendarEventsForAuth,
  grantCreatedEventToRestrictedUser,
  VALID_SCOPES,
  SESSION_PREFIX,
  INTEGRATION_PREFIX,
};
