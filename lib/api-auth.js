...
async function getAccessRecord(pool, neonUserId, email) {
  ...
  try {
    ...
    if (isUndefinedColumnError(err, 'is_catering')) {
      try {
        const r = await runSelect(columnsWithManager);
        const row = r.rows[0] || null;
        if (row) row.is_catering = false;
        return row;
      } catch (err2) {
        ...
        const row = r.rows[0] || null;
        if (row) {
          row.is_event_manager = false;
          row.is_catering = false;
        }
        return row;
      }
    }
    if (!isUndefinedColumnError(err, 'is_event_manager')) throw err;
    const r = await runSelect(columnsLegacy);
    const row = r.rows[0] || null;
    if (row) {
      row.is_event_manager = false;
...
function authorizeAccessManagerRoute(req, res, requireAdminAuth) {
  ...
  res.status(403).json({
    error: 'Forbidden',
    message: 'Administrator or event manager access required.',
  });
  return false;
}
  ...
  app.get('/api/admin/access-requests', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        if (isUndefinedColumnError(joinErr, 'is_catering')) {
      ...
      res.json({ requests, eventAccessNeedsMigration, dashboardNeedsMigration, cateringNeedsMigration });
    } catch (err) {
