...
const { notifyAdminsNewAccessRequest, notifyUserAccessApproved, notifyUserAccessRejected } = require('./admin-notify-email');
const {
...
function neonAuthOriginError(result) {
  ...
  return {
    ...
    hint: `Add "${result.origin}" in Neon Console → Auth → Configuration → Domains, or set NEON_AUTH_CLIENT_ORIGIN on Railway to an allowed origin (e.g. http://localhost:3003).`,
...
function authMigrationErrorPayload(err) {
  ...
  return {
    ...
    databaseHost,
    hint:
      `Run migrations/026, 027, and 028 in Neon SQL Editor on branch ${databaseHost || '(NEON_DATABASE_URL host)'}. ` +
  ...
  app.get('/api/auth/setup-status', async (req, res) => {
    try {
      ...
      return res.json({
        ...
        userFlow: {
          signUp: 'POST /api/auth/neon-register (Request access tab)',
