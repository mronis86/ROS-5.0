...
async function getAccessRecord(pool, neonUserId, email) {
  ...
  try {
    ...
    if (!isUndefinedColumnError(err, 'is_event_manager')) {
    ...
    if (row) row.is_event_manager = false;
    return row;
  }
}
...
function registerAuthRoutes(app, pool, options = {}) {
  ...
  app.get('/api/admin/access-requests', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        if (isUndefinedColumnError(joinErr, 'is_event_manager')) {
  ...
  app.patch('/api/admin/access-requests/:id', async (req, res) => {
    ...
    try {
      ...
      if (!isFullAdmin) {
        if (body.is_admin !== undefined || body.dashboard_enabled !== undefined || body.is_event_manager !== undefined) {
