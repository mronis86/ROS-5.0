  ...
  app.post('/api/auth/request-access', async (req, res) => {
    try {
      ...
      return res.status(201).json({
        ...
        message:
          row.status === 'approved'
            ? 'You are the first user and were approved as administrator. Save your personal link below to set your password.'
            : 'Access request submitted. Save your personal link below — you will need it after an admin approves you.',
        portalUrl,
      });
    } catch (err) {
      if (err.code === '42P01') {
  ...
  app.delete('/api/admin/access-requests/:id', async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        hint: 'Neon Auth login (if any) must be removed separately in Neon Console.',
      });
    } catch (err) {
      if (err.code === '42P01') {
        return res.status(503).json({ error: 'Run migration 027 on Neon for access approval.' });
