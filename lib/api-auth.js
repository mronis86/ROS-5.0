...
const { validateNeonAuthToken, isNeonAuthConfigured } = require('./neon-auth-server');
    ...
    try {
      ...
      if (!auth || auth.type !== 'neon_user') {
        return res.status(401).json({
          ...
          hint: isNeonAuthConfigured()
            ? 'Send Authorization: Bearer <Neon JWT from authClient.token()>, not the opaque session token.'
      ...
      const r = await pool.query(
        `INSERT INTO public.api_user_access (neon_user_id, email, full_name, status, is_admin)
         ...
         RETURNING id, status, is_admin, requested_at`,
        [auth.userId, auth.email, fullName || auth.email.split('@')[0], isFirstAdmin ? 'approved' : 'pending', isFirstAdmin]
      );
