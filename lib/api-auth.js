...
function pathRequiresAuth(pathname, method, config) {
  ...
  // Apps Script / schedulers use ADMIN_KEY via X-Admin-Key (checked in route by requireAdminAccess)
  if (pathname === '/api/backup/upcoming-export') return false;
...
function registerAuthRoutes(app, pool, options = {}) {
  ...
  app.post('/api/admin/access-requests/:id/approve', async (req, res) => {
    ...
    try {
      const { id } = req.params;
      const makeAdmin = req.auth?.isAdmin ? !!(req.body && req.body.make_admin) : false;
