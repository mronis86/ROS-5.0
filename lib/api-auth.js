...
} = require('./integration-token-vault');
const {
  hashPortalToken,
  generatePortalToken,
  isPortalToken,
...
function registerAuthRoutes(app, pool, options = {}) {
  ...
  app.get('/api/admin/integration-tokens', async (req, res) => {
    ...
    try {
      const r = await pool.query(
        `SELECT id, name, token_prefix, scopes, event_id, expires_at, revoked_at, created_at, last_used_at, token_ciphertext
         FROM public.api_integration_tokens
         ORDER BY created_at DESC`
      );
      const tokens = r.rows.map((row) => {
        const { token_ciphertext, ...rest } = row;
        const token =
          !rest.revoked_at && token_ciphertext
            ? decryptIntegrationToken(token_ciphertext)
            : null;
        return { ...rest, token };
      });
      res.json({ tokens, vaultEnabled: canStoreIntegrationTokens() });
    } catch (err) {
      ...
      if (err.code === '42703') {
        ...
        return res.json({
          tokens: r.rows.map((row) => ({ ...row, token: null })),
          needsTokenVaultMigration: true,
          vaultEnabled: false,
        });
      }
  ...
  app.post('/api/admin/integration-tokens', async (req, res) => {
    ...
    try {
      ...
      // created_by FK references legacy api_users.id — Neon Auth admins use accessId, not api_users
      const createdBy = req.auth?.type === 'user' && req.auth.userId ? req.auth.userId : null;

      let tokenCiphertext = null;
      if (canStoreIntegrationTokens()) {
        try {
          tokenCiphertext = encryptIntegrationToken(rawToken);
        } catch (vaultErr) {
          console.warn('[integration-tokens POST] vault encrypt failed:', vaultErr.message);
        }
      }
      ...
      try {
        r = await pool.query(
          `INSERT INTO public.api_integration_tokens (name, token_hash, token_prefix, scopes, event_id, created_by, expires_at, token_ciphertext)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
           RETURNING id, name, token_prefix, scopes, event_id, expires_at, created_at`,
          [tokenName, tokenHash, prefix, scopeList, eventId || null, createdBy, expiresAt, tokenCiphertext]
        );
      } catch (insertErr) {
      ...
      res.status(201).json({
        token: rawToken,
        record: { ...r.rows[0], token: rawToken },
        message: tokenCiphertext
          ? 'Token saved — use Copy token in the list anytime (admin only).'
          : 'Copy this token now — run migration 035 on Neon to enable re-copy from Admin.',
      });
    } catch (err) {
      if (err.code === '42P01') {
        return res.status(503).json({ error: 'Auth tables not found. Run migration 026 on Neon.' });
