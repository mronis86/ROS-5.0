  ...
  try {
    ...
    if (isUndefinedColumnError(err, 'is_bts_crew') || isUndefinedColumnError(err, 'is_comms')) {
      try {
        ...
        return withDefaults(r.rows[0] || null, { is_bts_crew: false, is_comms: false });
      } catch (errBts) {
    ...
    if (isUndefinedColumnError(err, 'is_catering') || isUndefinedColumnError(err, 'is_bts_crew') || isUndefinedColumnError(err, 'is_comms')) {
      try {
        ...
        return withDefaults(r.rows[0] || null, { is_catering: false, is_bts_crew: false, is_comms: false });
      } catch (err2) {
  ...
  app.get('/api/admin/access-requests', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        if (isUndefinedColumnError(joinErr, 'is_comms')) {
        ...
        r = await client.query(
          `UPDATE public.api_user_access
           SET status = 'approved',
               ...
               is_catering = $5,
               is_bts_crew = $6,
               reviewed_at = NOW(), reviewed_by = $7
           WHERE id = $1
           RETURNING *`,
          [id, makeAdmin, isEventManager, isComms, isCatering, isCrew, reviewedBy]
        );
      } catch (roleErr) {
        // Older DBs without newer role columns — fall back to admin-only approve.
        if (isUndefinedColumnError(roleErr, 'is_comms') || isUndefinedColumnError(roleErr, 'is_bts_crew') || isUndefinedColumnError(roleErr, 'is_catering') || isUndefinedColumnError(roleErr, 'is_event_manager')) {
          r = await client.query(
            `UPDATE public.api_user_access
