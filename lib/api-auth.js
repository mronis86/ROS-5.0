...
const { validateNeonAuthToken, isNeonAuthConfigured, resolveNeonIdentity, getNeonAuthBaseUrl } = require('./neon-auth-server');
...
function registerAuthRoutes(app, pool, options = {}) {
  ...
  app.post('/api/auth/neon-exchange', async (req, res) => {
    try {
      ...
      const identity = await resolveNeonIdentity(rawToken, sessionToken, bodyJwt);
      if (!identity) {
        return res.status(401).json({
