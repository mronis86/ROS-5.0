    ...
    try {
      ...
      try {
        r =
          filter === 'all'
            ? await pool.query(
                `SELECT a.id, a.neon_user_id, a.email, a.full_name, a.status, a.is_admin, a.dashboard_enabled, a.requested_at, a.reviewed_at, a.reviewed_by, a.notes, a.password_set_at, a.portal_token,
                        COALESCE(ea_counts.n, 0)::int AS event_access_count
                 FROM public.api_user_access a
                 LEFT JOIN (
      ...
      res.json({ requests, eventAccessNeedsMigration });
    } catch (err) {
  ...
  app.patch('/api/admin/access-requests/:id', async (req, res) => {
    ...
    try {
      ...
      if (body.dashboard_enabled !== undefined) {
        ...
        values.push(!!body.dashboard_enabled);
      }
