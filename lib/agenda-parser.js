...
const TIME_LINE_REGEX = /^\s*[•\-]\s*(\d{1,2}\s*:\s*\d{2}(?:\s*:\s*\d{2})?)\s*(AM|PM|am|pm)?(?:\s*[-–—]\s*(\d{1,2}\s*:\s*\d{2}(?:\s*:\s*\d{2})?)\s*(AM|PM|am|pm)?)?\s*$/i;

const TIME_LINE_REGEX_NO_BULLET = /^\s*(\d{1,2}\s*:\s*\d{2}(?:\s*:\s*\d{2})?)\s*(AM|PM|am|pm)?(?:\s*[-–—]\s*(\d{1,2}\s*:\s*\d{2}(?:\s*:\s*\d{2})?)\s*(AM|PM|am|pm)?)?\s*$/i;

// Line starts with optional bullet/dash, then time, then optional title (e.g. "9:00 AM Opening", "• 12:30pm Lunch")
const STARTS_WITH_TIME = /^\s*[•\-]?\s*(\d{1,2})\s*:\s*(\d{2})(?:\s*:\s*(\d{2}))?\s*(AM|PM|am|pm)?\s*[-–—]?\s*(.*)/i;
...
function buildBlocks(lines, overrideStartIndex) {
  ...
  for (let i = 0; i < fromFirstTime.length; i++) {
    ...
    if (timeOnly) {
      ...
      const m = trimmed.replace(/^\s*[•\-]\s*/, '').match(/(\d{1,2})\s*:\s*(\d{2})(?:\s*:\s*(\d{2}))?\s*(AM|PM|am|pm)?/i);
      const timeStr = m ? buildTimeStringFromMatch(m) : trimmed;
      const endPart = trimmed.match(/[-–—]\s*(\d{1,2}\s*:\s*\d{2}(?:\s*:\s*\d{2})?)\s*(AM|PM|am|pm)?/i);
