...
async function loadGuestEventPayload(pool, eventId) {
  ...
  try {
    activeTimer = await fetchGuestActiveTimer(pool, eventId);
  } catch (err) {
    if (err.code !== '42P01') {
      throw err;
    }
...
async function fetchGuestActiveTimer(pool, eventId) {
  ...
  return {
    ...
    cueIs: t.cue_is || '',
    startedAt: t.started_at instanceof Date ? t.started_at.toISOString() : t.started_at || null,
  };
}

/** Socket room for read-only guest viewers (never join operator event: room). */
function guestEventRoom(eventId) {
  return `guest-event:${String(eventId)}`;
}

/** Update types mirrored to guest-event rooms (whitelist only). */
const GUEST_SOCKET_TYPES = new Set([
  'timerUpdated',
  'timerStopped',
  'timersStopped',
  'runOfShowDataUpdated',
  'indentedCuesUpdated',
  'resetAllStates',
]);

function sanitizeGuestTimerRow(row) {
  ...
  return {
    itemId,
    timerState: row.timer_state ?? row.timerState ?? null,
    isActive: !!(row.is_active ?? row.isActive),
    isRunning: !!(row.is_running ?? row.isRunning),
    durationSeconds: row.duration_seconds ?? row.durationSeconds,
    elapsedSeconds: Number(row.elapsed_seconds ?? row.elapsedSeconds) || 0,
    cueIs: row.cue_is ?? row.cueIs ?? '',
    startedAt:
      startedAt instanceof Date ? startedAt.toISOString() : startedAt ? String(startedAt) : null,
  };
}

function sanitizeGuestSocketPayload(updateType, data) {
  switch (updateType) {
    case 'timerUpdated':
    case 'timerStopped':
      return sanitizeGuestTimerRow(data);
    case 'timersStopped':
      return { cleared: true, count: data?.count ?? 0 };
    case 'runOfShowDataUpdated': {
      const items = parseJsonMaybe(data?.schedule_items, []);
      const settings = parseJsonMaybe(data?.settings, {}) || {};
      ...
      return {
        ...
        masterStartTime: typeof settings.masterStartTime === 'string' ? settings.masterStartTime : '',
        dayStartTimes:
          settings.dayStartTimes && typeof settings.dayStartTimes === 'object'
            ? settings.dayStartTimes
            : {},
        numberOfDays: Number(settings.numberOfDays) || 1,
      };
    }
    case 'indentedCuesUpdated':
      if (data?.cleared) return { cleared: true, count: data.count ?? 0 };
      if (data?.removed) return { removed: true, itemId: data.itemId ?? data.item_id };
      if (data?.item_id != null) return { itemId: data.item_id, indented: true };
      return data;
    case 'resetAllStates':
      return { reset: true };
    default:
...
function emitGuestSocketUpdate(io, eventId, updateType, rawData) {
  if (!io || !GUEST_SOCKET_TYPES.has(updateType)) return;
  const data = sanitizeGuestSocketPayload(updateType, rawData);
  io.to(guestEventRoom(eventId)).emit('update', {
    type: updateType,
    data,
...
/** Lightweight poll payload — timer + server time only (no schedule). */
async function loadGuestLiveState(pool, eventId) {
  let activeTimer = null;
  try {
    activeTimer = await fetchGuestActiveTimer(pool, eventId);
  } catch (err) {
...
module.exports = {
  ...
  loadGuestLiveState,
  fetchGuestActiveTimer,
  sanitizeScheduleItem,
  sanitizeGuestTimerRow,
  sanitizeGuestSocketPayload,
