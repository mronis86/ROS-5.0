 ...
 *   OPS_ALERT_UNAUTHORIZED_SPA_MAX=500  — higher bar for Netlify/CF Pages/OBS/localhost (stale session)
 *   OPS_ALERT_UNAUTHORIZED_SPA_QUARANTINE=25 — after this many SPA 401s per IP+path, return 429 and stop counting
 *   OPS_ALERT_TRUSTED_REFERER_HOSTS=    — optional comma-separated extra SPA hosts (custom domains)
 */
...
/** IP+path quarantine after repeated SPA/unauthorized hits — middleware returns 429. */
const unauthorizedQuarantine = new Map();
...
function unauthorizedKey(req, pathname, spaNoise) {
  const ip = clientIp(req);
  return `${ip}:${pathname || requestPath(req)}:${spaNoise ? 'spa' : 'ext'}`;
}

/**
 * True when this IP+path was quarantined for repeated unauthorized SPA (or external) hits.
 * Middleware should return 429 and skip auth work.
 */
function isUnauthorizedQuarantined(req, pathname) {
  const spaNoise = isFirstPartySpaClient(req);
  const key = unauthorizedKey(req, pathname, spaNoise);
  const entry = unauthorizedQuarantine.get(key);
  if (!entry) return false;
  if (entry.until <= Date.now()) {
    unauthorizedQuarantine.delete(key);
    return false;
  }
  return true;
}

function setUnauthorizedQuarantine(key, untilMs) {
  unauthorizedQuarantine.set(key, { until: untilMs });
}

function quarantineRetryAfterSec(req, pathname) {
  const spaNoise = isFirstPartySpaClient(req);
  const key = unauthorizedKey(req, pathname, spaNoise);
  const entry = unauthorizedQuarantine.get(key);
  if (!entry) return 60;
...
/**
 * Our Netlify / Cloudflare Pages / localhost SPA (including OBS Browser Sources)
 * hitting protected routes with a missing OR expired session. Not an external
 * scanner — referer + normal browser/OBS UA. Spoofable, so we still count, but
 * email only at a much higher threshold.
 */
function isFirstPartySpaClient(req) {
  const ua = String(req?.headers?.['user-agent'] || '');
  if (!/mozilla\/\d/i.test(ua)) return false;
  const host = requestRefererHost(req);
  if (!host) return false;
  if (host === 'localhost' || host === '127.0.0.1') return true;
  // Dual hosting: Netlify + Cloudflare Pages (incl. preview *.pages.dev)
...
function userAgentHint(req) {
  const ua = String(req?.headers?.['user-agent'] || '');
  if (/OBS\//i.test(ua)) return 'OBS Browser Source';
  if (/Edg\//i.test(ua)) return 'Edge';
  if (/Chrome\//i.test(ua)) return 'Chrome';
  return ua ? ua.slice(0, 80) : undefined;
}

function recordUnauthorizedApiAccess(pool, req, pathname) {
  if (isOpsAlertsDisabled()) return { quarantined: false };

  const windowMin = readPositiveInt('OPS_ALERT_UNAUTHORIZED_API_WINDOW_MIN', 15);
  // Expired Bearer from OBS/SPA must count as spa noise too (not only missing token).
  const spaNoise = isFirstPartySpaClient(req);
  const threshold = spaNoise
    ? readPositiveInt('OPS_ALERT_UNAUTHORIZED_SPA_MAX', 500)
    : readPositiveInt('OPS_ALERT_UNAUTHORIZED_API_MAX', 10);
  const quarantineAt = spaNoise
    ? readPositiveInt('OPS_ALERT_UNAUTHORIZED_SPA_QUARANTINE', 25)
    : readPositiveInt('OPS_ALERT_UNAUTHORIZED_API_MAX', 10);
  const ip = clientIp(req);
  const key = unauthorizedKey(req, pathname, spaNoise);
  const now = Date.now();
  ...
  for (const [qKey, qEntry] of unauthorizedQuarantine) {
    if (!qEntry || qEntry.until <= now) unauthorizedQuarantine.delete(qKey);
  }
  ...
  if (entry.count >= quarantineAt) {
    setUnauthorizedQuarantine(key, entry.resetAt);
    quarantined = true;
  }

  if (entry.count === threshold) {
    const uaHint = userAgentHint(req);
    void queueOpsAlert(pool, {
      category: 'security_unauthorized_api',
      severity: 'warning',
      title: `Repeated unauthorized API access (${ip})`,
      summary: spaNoise
        ? `${entry.count} unauthorized requests from our web app/OBS (${uaHint || 'browser'}) — missing or expired session — in the last ${windowMin} minutes. Usually a stale display tab or OBS Browser Source, not an external scan. Pause Displays for the event on the Event List (or Admin → Timers → Kill display sync) and close/refresh stale OBS/Edge tabs.`
      ...
      details: {
        ...
        clientHint: uaHint,
        hadBearerToken: hasBearerOrQueryToken(req) || undefined,
        firstPartySpaClient: spaNoise || undefined,
  ...
  return { quarantined, count: entry.count };
}
