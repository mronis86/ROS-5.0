...
function recordUnauthorizedApiAccess(pool, req, pathname) {
  ...
  const threshold = readPositiveInt('OPS_ALERT_UNAUTHORIZED_API_MAX', 10);
  const ip = clientIp(req);
  const key = `${ip}:${pathname || requestPath(req)}`;
  const now = Date.now();
  ...
  void queueOpsAlert(pool, {
    ...
    summary: `${entry.count} unauthorized requests to protected API routes from this IP in the last ${windowMin} minutes.`,
    details: {
      ip,
