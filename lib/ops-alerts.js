...
function recordUnauthorizedApiAccess(pool, req, pathname) {
  ...
  void queueOpsAlert(pool, {
    ...
    summary: spaNoise
      ? `${entry.count} unauthorized requests from our web app/OBS (${uaHint || 'browser'}) — missing or expired session — in the last ${windowMin} minutes. Usually a stale display tab or OBS Browser Source, not an external scan.`
