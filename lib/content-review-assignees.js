...
function canManageContentReviewAssignees(auth) {
  ...
  if (auth.isAdmin || auth.isEventManager || auth.isBtsCrew) return true;
  if (auth.isCreative) return false;
  if (auth.isComms || auth.isCatering) return false;
  return true;
}
...
async function validateAssigneeIds(pool, role, accessIds) {
  ...
  const r = await pool.query(
    `SELECT id, email, full_name, is_creative, is_admin, is_event_manager, status
     FROM public.api_user_access
     WHERE id = ANY($1::uuid[])`,
    [unique]
  );
  ...
  for (const id of unique) {
    ...
    if (role === 'creative' && !user.is_creative && !user.is_admin && !user.is_event_manager) {
      return { error: `${user.full_name || user.email} is not a Creative user.` };
    }
    if (role === 'production' && user.is_creative && !user.is_admin && !user.is_event_manager) {
      return { error: `${user.full_name || user.email} is a Creative-only user — assign as Creative instead.` };
    }
  }
  return { ids: unique };
}
