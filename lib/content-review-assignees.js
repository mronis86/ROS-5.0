...
async function validateAssigneeIds(pool, role, accessIds) {
  ...
  for (const id of unique) {
    ...
    if (role === 'creative' && !user.is_creative) {
      return { error: `${user.full_name || user.email} is not a Creative user.` };
    }
    if (role === 'production' && user.is_creative) {
      return { error: `${user.full_name || user.email} is a Creative user — assign under Creative reviewers instead.` };
    }
  }
  return { ids: unique };
}
