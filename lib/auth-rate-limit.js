 ...
 *   AUTH_RATE_LIMIT_LOGIN_WARNING_AFTER=5    — show client warning after this many failures
...
function loginWindowMinutes() {
  return readPositiveInt('AUTH_RATE_LIMIT_LOGIN_WINDOW_MIN', 15);
}

function loginAttemptLimit() {
  return readPositiveInt('AUTH_RATE_LIMIT_LOGIN_MAX', 8);
}

function loginWarningAfterAttempts() {
  return readPositiveInt('AUTH_RATE_LIMIT_LOGIN_WARNING_AFTER', 5);
}

function lockoutMinutesFromReset(resetTime) {
  const windowMin = loginWindowMinutes();
  if (!resetTime) return windowMin;
  return Math.max(1, Math.ceil(Math.max(0, resetTime - Date.now()) / 60000));
}

function getLoginRateLimitInfo(req) {
  const rl = req.rateLimit;
  if (!rl || typeof rl.remaining !== 'number') return {};
  return {
    login_attempts_remaining: rl.remaining,
    login_attempts_limit: rl.limit,
    lockout_minutes: loginWindowMinutes(),
    login_warning_after: loginWarningAfterAttempts(),
  };
}

function credentialRateLimitResponse(req, res, pool) {
  const rl = req.rateLimit;
  const lockoutMin = lockoutMinutesFromReset(rl?.resetTime);
  const email = normalizeEmail(req.body?.email);
  if (pool && email) {
    const { queueLoginSecurityAdminAlerts } = require('./login-security-alerts');
    queueLoginSecurityAdminAlerts(pool, req, email, { forceStage: 'lockout' }).catch((err) =>
      console.error('[login-security-alerts]', err.message)
    );
  }
  res.status(429).json({
    error: `Too many sign-in attempts. Please wait ${lockoutMin} minutes and try again.`,
    code: 'rate_limit_exceeded',
    login_attempts_remaining: 0,
    login_attempts_limit: rl?.limit ?? loginAttemptLimit(),
    lockout_minutes: lockoutMin,
    login_warning_after: loginWarningAfterAttempts(),
  });
}
...
function buildCredentialLimiter(pool) {
  const windowMin = loginWindowMinutes();
  return rateLimit({
    ...
    legacyHeaders: false,
    handler: (req, res) => credentialRateLimitResponse(req, res, pool),
  });
}
...
function applyAuthRateLimits(app, pool) {
  if (isRateLimitDisabled()) {
    console.log('[auth-rate-limit] disabled');
    return;
  }

  const credentialLimiter = buildCredentialLimiter(pool);
