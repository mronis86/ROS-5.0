...
/** Neon Auth auto-trusts localhost; map LAN dev URLs (192.168.x.x) to localhost for server-side sign-in. */
function normalizeNeonClientOrigin(origin) {
  const fromEnv = (process.env.NEON_AUTH_CLIENT_ORIGIN || '').trim();
  if (fromEnv) return fromEnv;
  try {
    const u = new URL(origin);
    if (!isPrivateLanHostname(u.hostname)) return origin;
    if (u.hostname === 'localhost' || u.hostname === '127.0.0.1') return origin;
    const port = u.port || '3003';
...
function resolveNeonClientOrigin(originHeader, refererHeader) {
  ...
  if (origin) return normalizeNeonClientOrigin(origin);
  const referer = (refererHeader || '').trim();
  if (referer) {
    try {
      return normalizeNeonClientOrigin(new URL(referer).origin);
    } catch {
      /* ignore */
    }
  }
...
async function neonAuthEmailRequest(path, body, clientOrigin) {
  const base = getNeonAuthBaseUrl();
  if (!base) {
    return { ok: false, status: 503, error: 'Neon Auth is not configured on the API server.' };
  }

  const origin = resolveNeonClientOrigin(clientOrigin, null);
  let res;
  try {
