...
module.exports = {
  ...
  parseNodeVersion,
  maybeNotifyPlatformMaintenance,
  startPlatformMaintenanceAlerts,
};

/**
 * Email admins when Platform maintenance has Review or Urgent items.
 * Uses ops-alert cooldown (default 24h) so Refresh in Admin does not spam.
 */
async function maybeNotifyPlatformMaintenance(pool, options = {}) {
  const { queueOpsAlert, isOpsAlertsDisabled } = require('./ops-alerts');
  if (isOpsAlertsDisabled()) return { sent: false, reason: 'ops_alerts_disabled' };

  const report = options.report || (await buildPlatformMaintenanceReport());
  const level = report.summary?.level;
  if (level !== 'warning' && level !== 'critical') {
    return { sent: false, reason: 'ok', level };
  }

  const attention = report.attention || [];
  const cooldownHours = Math.max(
    1,
    parseInt(process.env.PLATFORM_MAINTENANCE_ALERT_COOLDOWN_HOURS || '168', 10) || 168
  );
  const issueKey = attention
    .map((c) => c.id)
    .sort()
    .join(',');

  const lines = attention.map((c) => {
    const dateBit = c.dateRight || c.recommendByLabel || '';
    return `${c.title}: ${c.plain || c.detail || ''}${dateBit ? ` (${dateBit})` : ''}${c.action ? ` → ${c.action}` : ''}`;
  });
  ...
  await queueOpsAlert(pool, {
    ...
    title:
      level === 'critical'
        ? 'Platform maintenance: urgent upgrade needed'
        : 'Platform maintenance: items to review',
    summary: report.summary.headline || 'Platform maintenance needs attention.',
    details: {
      level,
      nodeVersion: report.runtime?.nodeVersion || process.version,
      env: report.runtime?.env || process.env.NODE_ENV || 'unknown',
      checkedAt: report.checkedAt,
      items: lines,
      tip: 'Open Admin → Platform for the full checklist. Dates refresh from endoflife.date.',
    },
    dedupeKey: `platform_maintenance:${level}:${issueKey}`,
    cooldownMinutes: cooldownHours * 60,
  });

  return { sent: true, level, issueKey, cooldownHours };
}

function startPlatformMaintenanceAlerts(pool) {
  if (!pool) return;
  const enabled =
    process.env.PLATFORM_MAINTENANCE_ALERTS_DISABLED === 'true'
      ? false
      : true;
  if (!enabled) {
    console.log('[platform-maintenance] Admin email alerts disabled via PLATFORM_MAINTENANCE_ALERTS_DISABLED');
    return;
  }

  const intervalHours = Math.max(
    1,
    parseInt(process.env.PLATFORM_MAINTENANCE_ALERT_INTERVAL_HOURS || '168', 10) || 168
  );
  const run = () => {
    maybeNotifyPlatformMaintenance(pool).catch((err) =>
      console.error('[platform-maintenance] alert check failed:', err.message || err)
    );
  };

  // First check a few minutes after boot (avoid deploy storm), then on interval
  setTimeout(run, 3 * 60 * 1000);
  setInterval(run, intervalHours * 60 * 60 * 1000);
  console.log(
    `[platform-maintenance] Admin email alerts scheduled (first check ~3m, then every ${intervalHours}h; cooldown via PLATFORM_MAINTENANCE_ALERT_COOLDOWN_HOURS)`
  );
}
