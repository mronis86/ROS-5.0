...
function buildStreamRequestSubmittedEmail({
  eventName,
  eventDate,
  eventId,
  editUrl,
  ...
  const channel =
    youtubeChannel === 'Other' && youtubeChannelOther
      ? `Other — ${youtubeChannelOther}`
      : youtubeChannel || '—';
  const when = requestSubmittedAt ? new Date(requestSubmittedAt) : new Date();
  const whenLabel = Number.isNaN(when.getTime())
    ? ''
    : `${when.toLocaleString('en-US', { timeZone: 'UTC' })} UTC`;
  const contact = [requestContactName, requestContactEmail].filter(Boolean).join(' · ') || '—';
  const subject = `Run of Show — stream request submitted (${eventName || 'event'})`;

  const lines = [
    'A Stream Request form was submitted.',
    '',
    ...
    `Video name: ${youtubeVideoTitle || '—'}`,
    youtubeDescription ? `Description: ${youtubeDescription}` : '',
    `Visibility: ${visibility || '—'}`,
    `Stream start time: ${streamStartTime || '—'}`,
    `Share player link with: ${shareWith || '—'}`,
    `Submitted by: ${contact}`,
    whenLabel ? `Submitted: ${whenLabel}` : '',
    editUrl ? '' : '',
    editUrl ? `Open Event List / edit the event to add RTMP + stream key:` : '',
    editUrl || '',
  ...
  const html = `<!DOCTYPE html>
<html><body style="font-family:sans-serif;line-height:1.5;color:#111">
  <p>A <strong>Stream Request</strong> form was submitted for Run of Show.</p>
  <table style="border-collapse:collapse;margin:16px 0">
    <tr><td style="padding:4px 12px 4px 0;color:#555">Event</td><td><strong>${escapeHtml(eventName || 'Untitled event')}</strong></td></tr>
    ${eventDate ? `<tr><td style="padding:4px 12px 4px 0;color:#555">Date</td><td>${escapeHtml(eventDate)}</td></tr>` : ''}
    <tr><td style="padding:4px 12px 4px 0;color:#555">YouTube channel</td><td>${escapeHtml(channel)}</td></tr>
    <tr><td style="padding:4px 12px 4px 0;color:#555">Video name</td><td>${escapeHtml(youtubeVideoTitle || '—')}</td></tr>
    ${
      youtubeDescription
        ? `<tr><td style="padding:4px 12px 4px 0;color:#555;vertical-align:top">Description</td><td>${escapeHtml(youtubeDescription).replace(/\n/g, '<br/>')}</td></tr>`
        : ''
    }
    <tr><td style="padding:4px 12px 4px 0;color:#555">Visibility</td><td>${escapeHtml(visibility || '—')}</td></tr>
    <tr><td style="padding:4px 12px 4px 0;color:#555">Stream start</td><td>${escapeHtml(streamStartTime || '—')}</td></tr>
    <tr><td style="padding:4px 12px 4px 0;color:#555;vertical-align:top">Share with</td><td>${escapeHtml(shareWith || '—').replace(/\n/g, '<br/>')}</td></tr>
    <tr><td style="padding:4px 12px 4px 0;color:#555">Submitted by</td><td>${escapeHtml(contact)}</td></tr>
    ${whenLabel ? `<tr><td style="padding:4px 12px 4px 0;color:#555">Submitted</td><td>${escapeHtml(whenLabel)}</td></tr>` : ''}
  </table>
  ${
    editUrl
      ? `<p>Open the <a href="${escapeHtml(editUrl)}">Event List</a> and edit this event to add the RTMP URL and stream key.</p>`
      : '<p>Open <strong>Event List</strong> and edit this event to add the RTMP URL and stream key.</p>'
  }
</body></html>`;
...
async function notifyAdminsStreamRequestSubmitted(pool, payload) {
  if (!isAdminEmailNotifyConfigured()) {
    skipEmail('RESEND_API_KEY or ADMIN_NOTIFY_FROM not set on the API server (Railway)');
    return;
  }
  ...
  const { subject, text, html } = buildStreamRequestSubmittedEmail(payload || {});
  const result = await sendViaResend({ to: recipients, subject, html, text });
  console.log(
    `[admin-notify-email] Sent stream request notice to ${recipients.length} admin(s): ${recipients.join(', ')}` +
      (result.resendId ? ` (resend id: ${result.resendId})` : '')
  );
}
...
module.exports = {
  ...
  buildOpsAlertEmail,
  buildStreamRequestSubmittedEmail,
