...
function buildTrainingBookingConfirmationEmail(booking, { timezone, icsUrl, origin } = {}) {
  ...
  const lines = [
    ...
    `When: ${when}`,
    '',
    icsUrl ? `Add to your calendar: ${icsUrl}` : '',
    'A calendar invite is attached — Gmail and Outlook should offer to add it to your calendar.',
    '',
  ...
  const eventSchema =
    booking.startsAt && booking.endsAt
      ? {
          ...
          '@type': 'Event',
          name: 'ROS Application Training',
          startDate: booking.startsAt,
          endDate: booking.endsAt,
          ...
          ...(origin
            ? {
                location: {
                  ...
                  url: `${origin.replace(/\/$/, '')}/training`,
                },
              }
            : {}),
          organizer: { '@type': 'Organization', name: 'Run of Show' },
        }
      : null;
...
<html><body style="font-family:sans-serif;line-height:1.5;color:#111">
  ...
  ${
    icsUrl
      ? `<p><a href="${escapeHtml(icsUrl)}">Add to your calendar</a> (or use the calendar invite attached to this email)</p>`
      : '<p>Use the calendar invite attached to this email to add this session to your calendar.</p>'
  }
...
/** Email the person who booked with confirmation + calendar invite attachment. */
async function notifyTrainingBookingConfirmation(booking, { timezone, icsUrl, icsFilename, icsBody, origin } = {}) {
  if (!isAdminEmailNotifyConfigured()) {
  ...
  const { subject, text, html } = buildTrainingBookingConfirmationEmail(booking, { timezone, icsUrl, origin });
  const attachments =
