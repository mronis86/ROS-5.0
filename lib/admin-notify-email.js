...
function buildContentReviewAssignedEmail({ fullName, email, eventName, eventId, role }) {
  ...
  const text = [
    ...
    url,
    '',
    'You will receive email when there are new comments or review status changes on this event.',
  ].join('\n');
  const html = `<!DOCTYPE html>
<html><body style="font-family:sans-serif;line-height:1.5;color:#111">
  <p>Hi <strong>${escapeHtml(name)}</strong>,</p>
  ...
  <p style="color:#555;font-size:13px">You will receive email when there are new comments or review updates on this event.</p>
</body></html>`;
  return { subject, text, html };
}
