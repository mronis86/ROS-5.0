...
function buildTrainingBookingConfirmationEmail(booking, { timezone, icsUrl } = {}) {
  ...
  const lines = [
    ...
    'A calendar file (.ics) is attached — open it to add this session to Google Calendar, Outlook, Apple Calendar, and others.',
    '',
...
<html><body style="font-family:sans-serif;line-height:1.5;color:#111">
  ...
  ${
    icsUrl
      ? `<p><a href="${escapeHtml(icsUrl)}">Add to your calendar</a> (or open the attached <strong>.ics</strong> file)</p>`
      : '<p>Open the attached <strong>.ics</strong> file to add this session to your calendar.</p>'
  }
