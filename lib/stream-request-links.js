...
async function loadStreamRequestFormPayload(pool, eventId) {
  ...
  return {
    ...
    alreadySubmitted: !!(
      ...
      streamDetails.youtubeVideoTitle ||
      streamDetails.youtubeDescription ||
      streamDetails.visibility ||
      streamDetails.shareWith ||
      streamDetails.streamStartTime ||
    ...
    existing: {
      ...
      youtubeVideoTitle: streamDetails.youtubeVideoTitle || '',
      youtubeDescription: streamDetails.youtubeDescription || '',
      visibility: streamDetails.visibility || '',
      streamStartTime: streamDetails.streamStartTime || '',
...
function normalizeStreamRequestBody(body) {
  ...
  const youtubeVideoTitle = String(body?.youtubeVideoTitle || body?.videoName || '').trim();
  const youtubeDescription = String(body?.youtubeDescription || body?.description || '').trim();
  const visibility = String(body?.visibility || '').trim();
  const streamStartTime = String(body?.streamStartTime || body?.startTime || '').trim();
  ...
  if (!youtubeVideoTitle) {
    return { error: 'Please enter the YouTube video name.' };
  }
  if (!VISIBILITY_OPTIONS.includes(visibility)) {
    return { error: 'Please choose Public or Unlisted for YouTube visibility.' };
  }
  if (!streamStartTime) {
    return { error: 'Please enter the stream start time.' };
  }
  if (!shareWith) {
...
async function applyStreamRequestToEvent(pool, eventId, requestFields) {
  ...
  const nextDetails = {
    ...
    youtubeVideoTitle: requestFields.youtubeVideoTitle,
    youtubeDescription: requestFields.youtubeDescription,
    visibility: requestFields.visibility,
    streamStartTime: requestFields.streamStartTime,
  ...
  return {
    ...
    eventName: calendar.name || 'Untitled event',
    eventDate:
      calendar.date instanceof Date
        ? `${calendar.date.getFullYear()}-${String(calendar.date.getMonth() + 1).padStart(2, '0')}-${String(calendar.date.getDate()).padStart(2, '0')}`
