...
const RELATED_TABLES = [
  ...
  { table: 'complaint_line_notes', column: 'event_id' },
  { table: 'catering_notes', column: 'event_id' },
  { table: 'run_of_show_backups', column: 'event_id' },
  { table: 'api_user_event_access', column: 'event_id' },
  { table: 'api_event_share_tokens', column: 'event_id' },
  { table: 'api_event_guest_links', column: 'event_id' },
  { table: 'api_quick_mode_operator_links', column: 'event_id' },
  { table: 'event_cue_files', column: 'event_id' },
];
...
/**
 * SQL: every id that still has a calendar_events row (active or soft-deleted).
 * Includes calendar UUID and legacy schedule_data.eventId / event_id links.
 */
const CALENDAR_LINK_SQL = `
  SELECT DISTINCT lower(trim(link_id)) AS link_id
  FROM (
    SELECT id::text AS link_id
    FROM calendar_events
    UNION ALL
    SELECT schedule_data->>'eventId'
    FROM calendar_events
    WHERE schedule_data IS NOT NULL
      AND COALESCE(schedule_data->>'eventId', '') <> ''
    UNION ALL
    SELECT schedule_data->>'event_id'
    FROM calendar_events
    WHERE schedule_data IS NOT NULL
      AND COALESCE(schedule_data->>'event_id', '') <> ''
  ) links
  WHERE link_id IS NOT NULL AND trim(link_id) <> ''
`;

/** Calendar event names still present (active or soft-deleted). */
const CALENDAR_NAME_SQL = `
  SELECT DISTINCT lower(trim(name)) AS event_name
  FROM calendar_events
  WHERE name IS NOT NULL AND trim(name) <> ''
`;

/**
 * True when this event_id is still represented on the calendar:
 * - direct id / schedule_data.eventId link, OR
 * - a calendar row with the same event name (covers re-keyed / duplicate ROS rows)
 *
 * IMPORTANT: pass fully-qualified expressions (e.g. r.event_id::text, r.event_name)
 * so nested subqueries do not shadow column names.
 */
function eventStillOnCalendarPredicate(eventIdExpr, eventNameExpr = null) {
  const idMatch = `
    EXISTS (
      SELECT 1 FROM (${CALENDAR_LINK_SQL}) cal
      WHERE cal.link_id = lower(trim(${eventIdExpr}))
    )
  `;
  if (!eventNameExpr) return idMatch;
  return `
    (
      ${idMatch}
      OR (
        ${eventNameExpr} IS NOT NULL
        AND trim((${eventNameExpr})::text) <> ''
        AND EXISTS (
          SELECT 1 FROM (${CALENDAR_NAME_SQL}) cn
          WHERE cn.event_name = lower(trim((${eventNameExpr})::text))
        )
      )
    )
  `;
...
async function loadCalendarIdSet(pool) {
  const result = await pool.query(CALENDAR_LINK_SQL);
  const ids = new Set();
  for (const row of result.rows || []) {
    if (row.link_id) ids.add(String(row.link_id));
  }
  return ids;
}

async function loadCalendarNameSet(pool) {
  const result = await pool.query(CALENDAR_NAME_SQL);
  const names = new Set();
  for (const row of result.rows || []) {
    if (row.event_name) names.add(String(row.event_name));
  }
  return names;
}

function isLinkedToCalendar(calendarIds, eventId) {
  const key = String(eventId || '').trim().toLowerCase();
  return key ? calendarIds.has(key) : true;
}

function nameStillOnCalendar(calendarNames, eventName) {
  const key = String(eventName || '').trim().toLowerCase();
  return key ? calendarNames.has(key) : false;
}
...
async function findOrphanEventData(pool) {
  // Exclude anything still represented on the calendar by id link OR same event name.
  const [calendarIds, calendarNames] = await Promise.all([
    loadCalendarIdSet(pool),
    loadCalendarNameSet(pool),
  ]);
  const orphanMap = new Map(); // eventId -> entry
  const stillOnCalendar = eventStillOnCalendarPredicate('r.event_id::text', 'r.event_name');
  ...
  for (const { table, column } of RELATED_TABLES) {
    ...
    // ROS: exclude id-linked rows and rows whose event_name is still on the calendar.
    if (table === 'run_of_show_data') {
      try {
        const r = await pool.query(
          `SELECT r.event_id::text AS eid,
                  r.event_name,
                  r.event_date,
                  r.settings,
                  r.schedule_items,
                  r.updated_at
           FROM run_of_show_data r
           WHERE r.event_id IS NOT NULL
             AND NOT (${stillOnCalendar})`
        );
        for (const row of r.rows || []) {
          const eid = String(row.eid || '').trim();
          if (!eid || isLinkedToCalendar(calendarIds, eid)) continue;
          if (nameStillOnCalendar(calendarNames, row.event_name)) continue;
    ...
    try {
      const pred = eventStillOnCalendarPredicate(`t.${column}::text`);
      const r = await pool.query(
        `SELECT t.${column}::text AS eid, COUNT(*)::int AS c
         FROM ${table} t
         WHERE t.${column} IS NOT NULL
           AND NOT (${pred})
         GROUP BY t.${column}::text`
      );
      for (const row of r.rows || []) {
        const eid = String(row.eid || '').trim();
        if (!eid || isLinkedToCalendar(calendarIds, eid)) continue;
  ...
  // Drop any leftover ids whose resolved name is still on the calendar (stale re-keys).
  for (const [id, entry] of [...orphanMap.entries()]) {
    if (nameStillOnCalendar(calendarNames, entry.name)) {
      orphanMap.delete(id);
    }
  }
...
async function purgeEventData(pool, { eventId, linkedIds = [], deleteCalendarRow = false } = {}) {
  ...
  try {
    const { deleteCueFilesForEventIds } = require('./event-cue-files');
    await deleteCueFilesForEventIds(pool, ids);
  } catch (err) {
    console.warn('[event-lifecycle] cue file bucket purge failed:', err.message || err);
  }
