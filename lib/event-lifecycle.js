...
/** True when event_id still maps to any calendar_events row (including soft-deleted). */
const HAS_CALENDAR_LINK_SQL = `
  EXISTS (
    SELECT 1 FROM (${CALENDAR_LINK_SQL}) cal
    WHERE cal.link_id = lower(trim($EVENT_ID_EXPR))
  )
`;

function hasCalendarLinkPredicate(eventIdExpr) {
  return HAS_CALENDAR_LINK_SQL.replace(/\$EVENT_ID_EXPR/g, eventIdExpr);
}
...
async function findOrphanEventData(pool) {
  // SQL anti-join against calendar UUID + schedule_data.eventId links (case-insensitive).
  const calendarIds = await loadCalendarIdSet(pool);
  const orphanMap = new Map(); // eventId -> entry
  const noCalendar = hasCalendarLinkPredicate('event_id::text');
  ...
  for (const { table, column } of RELATED_TABLES) {
    ...
    // Pull ROS metadata in the same query; only rows with NO calendar link.
    ...
    try {
      const pred = hasCalendarLinkPredicate(`${column}::text`);
      const r = await pool.query(
        `SELECT ${column}::text AS eid, COUNT(*)::int AS c
         FROM ${table}
