...
async function readAppSettingsRow(pool) {
  try {
    const r = await pool.query(
      'SELECT logo_variant_id, green_room_layout_id, countdown_color_mode, updated_at FROM public.app_settings WHERE id = 1'
    );
    if (r.rows.length === 0) {
      return {
    ...
    if (isMissingGreenRoomColumnError(err) || isMissingCountdownColorColumnError(err)) {
...
function registerAppSettingsRoutes(app, pool, { requireAdminAccess }) {
  ...
  app.put('/api/admin/app-settings', async (req, res) => {
    ...
    try {
      ...
      await pool.query(
        `INSERT INTO public.app_settings (id, logo_variant_id, green_room_layout_id, countdown_color_mode, updated_at)
         VALUES (1, $1, $2, $3, NOW())
         ...
         SET logo_variant_id = EXCLUDED.logo_variant_id,
             ...
             updated_at = NOW()`,
        [nextLogo, nextGreen, nextCountdown]
      );
      const settings = await readAppSettingsRow(pool);
      res.json(settingsPayload(settings));
    } catch (err) {
