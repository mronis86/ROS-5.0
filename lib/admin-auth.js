...
function getAdminKeyFromRequest(req) {
  ...
  if (typeof authHeader === 'string' && authHeader.startsWith('Bearer ')) {
    ...
    // Session/integration tokens use Bearer too — only treat other values as ADMIN_KEY.
    if (
      bearer &&
      !bearer.startsWith('ros_nsess_') &&
      !bearer.startsWith('ros_sess_') &&
      !bearer.startsWith('ros_itok_')
    ) {
      return bearer;
    }
  }
...
function createRequireAdminAuth(adminKey) {
  return function requireAdminAuth(req, res) {
    ...
    if (!timingSafeStringEqual(key, adminKey)) {
      const { recordAdminAuthFailure } = require('./ops-alerts');
      recordAdminAuthFailure(null, req);
...
/** Shared ADMIN_KEY or approved Neon user with is_admin (req.auth from api-auth middleware). */
function createRequireAdminAccess(adminKey) {
  const requireAdminKey = createRequireAdminAuth(adminKey);
  return function requireAdminAccess(req, res) {
