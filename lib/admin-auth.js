...
function getAdminPinFromRequest(req) {
  ...
  if (typeof headerPin === 'string') return headerPin;
  if (typeof req.query.pin === 'string') return req.query.pin;
  return '';
}
...
function createRequireAdminAuth(adminKey, adminPin) {
  return function requireAdminAuth(req, res) {
    ...
    if (adminPin && !timingSafeStringEqual(getAdminPinFromRequest(req), adminPin)) {
      res.status(401).json({ error: 'Unauthorized' });
      return false;
    }
    return true;
