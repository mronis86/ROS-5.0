...
async function ensureTrainingSchema(pool) {
  ...
  `);
  // Multiple people may book the same hour — remove exclusive unique index if it exists.
...
async function listBookedCounts(pool, fromUtc, toUtc) {
  ...
  for (const row of r.rows) {
    ...
    map.set(d.toISOString(), Number(row.count) || 0);
  }
  return map;
}
...
async function listAvailableSlots(pool, { fromDateKey, days } = {}) {
  ...
  const [blocked, bookedCounts] = await Promise.all([
    ...
    if (isWeekdayParts(cursor.year, cursor.month, cursor.day, tz) && !blockedSet.has(dateKey)) {
      for (let hour = SLOT_START_HOUR; hour <= SLOT_END_HOUR; hour++) {
        ...
        slots.push({
          ...
          date: dateKey,
          hour,
          bookingCount: bookedCounts.get(startsIso) || 0,
