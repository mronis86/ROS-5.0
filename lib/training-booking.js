...
function buildTrainingIcs(booking, { origin, mode = 'publish', organizerEmail } = {}) {
  const id = booking.id;
  ...
  const bookerName = String(booking.name || 'Guest').trim() || 'Guest';
  const bookerEmail = String(booking.email || '').trim().toLowerCase();
  const descriptionLines = [
    `Training session booked for ${bookerName}`,
  ...
  const isInvite = mode === 'request' && organizerEmail && bookerEmail;
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//ROS//Training Booking//EN',
    'CALSCALE:GREGORIAN',
    isInvite ? 'METHOD:REQUEST' : 'METHOD:PUBLISH',
    'BEGIN:VEVENT',
    `UID:${uid}`,
  ...
  if (origin) {
    lines.push(`URL:${escapeIcsText(`${origin}/training`)}`);
    lines.push(`LOCATION:${escapeIcsText(`Online — ${origin}/training`)}`);
  }
  if (isInvite) {
    lines.push(`ORGANIZER;CN=Run of Show Training:mailto:${organizerEmail}`);
    lines.push(
      `ATTENDEE;CUTYPE=INDIVIDUAL;ROLE=REQ-PARTICIPANT;PARTSTAT=ACCEPTED;RSVP=FALSE;CN=${escapeIcsText(bookerName)}:mailto:${bookerEmail}`
    );
  }
...
function getTrainingOrganizerEmail() {
  const candidates = [
    process.env.TRAINING_NOTIFY_EMAIL,
    process.env.ADMIN_PRIMARY_EMAIL,
...
function buildTrainingInviteIcs(booking, { origin } = {}) {
  return buildTrainingIcs(booking, {
    origin,
    mode: 'request',
    organizerEmail: getTrainingOrganizerEmail(),
  });
}
...
async function listBookingsAdmin(pool, { includeCancelled = false, limit = 200 } = {}) {
  const lim = Math.min(Math.max(Number(limit) || 200, 1), 500);
  const r = await pool.query(
    includeCancelled
      ? `SELECT * FROM public.training_bookings ORDER BY starts_at DESC LIMIT $1`
      : `SELECT * FROM public.training_bookings WHERE cancelled_at IS NULL ORDER BY starts_at ASC LIMIT $1`,
    [lim]
  );
