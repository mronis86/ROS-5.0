 ...
 * One active booking per starts_at; whole dates can be blocked by admin.
 */
...
function isMissingTrainingTableError(err) {
  return (
    err &&
    (err.code === '42P01' ||
      /training_bookings|training_blocked_dates/i.test(String(err.message || '')))
  );
}
...
async function listBookedStarts(pool, fromUtc, toUtc) {
  const r = await pool.query(
    `SELECT starts_at FROM public.training_bookings
     WHERE cancelled_at IS NULL
...
async function listAvailableSlots(pool, { fromDateKey, days } = {}) {
  ...
  const [blocked, booked] = await Promise.all([
    ...
    listBookedStarts(pool, rangeStart, rangeEnd),
  ]);
  ...
  for (let i = 0; i <= horizon; i++) {
    ...
    if (isWeekdayParts(cursor.year, cursor.month, cursor.day, tz) && !blockedSet.has(dateKey)) {
      for (let hour = SLOT_START_HOUR; hour <= SLOT_END_HOUR; hour++) {
        ...
        if (booked.has(startsIso)) continue;
...
async function createBooking(pool, payload) {
  ...
  try {
    ...
    if (err.code === '23505') {
      const conflict = new Error('That time was just booked by someone else. Please pick another slot.');
      conflict.status = 409;
      throw conflict;
    }
    throw err;
