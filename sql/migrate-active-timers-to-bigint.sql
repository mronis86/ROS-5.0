-- Migration script to change item_id from INTEGER to BIGINT
-- This handles the case where item IDs are timestamp-based and exceed INTEGER range

-- First, drop the existing table if it exists (since we're in development)
DROP TABLE IF EXISTS active_timers CASCADE;

-- Recreate the table with BIGINT for item_id
...
CREATE INDEX idx_active_timers_started_at ON active_timers(started_at);
