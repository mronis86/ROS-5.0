      ...
      try {
        ...
        if (activeTimer && supabaseOnly) {
          ...
          if (activeTimer.is_running && activeTimer.is_active && activeTimer.started_at) {
            ...
            try {
              console.log(`🔄 Clock DriftDetector: Starting initial sync for timer ${currentItemId}`);
              await driftDetector.forceSync(currentItemId, async () => {
                const activeTimerData = await DatabaseService.getActiveTimer(eventId);
                const serverElapsed = activeTimerData?.elapsed_seconds || 0;
                console.log(`🔄 Clock DriftDetector: Initial server elapsed: ${serverElapsed}s for timer ${currentItemId}`);
                return serverElapsed;
              });
            } catch (error) {
              console.warn(`⚠️ Clock DriftDetector: Failed initial sync for timer ${currentItemId}:`, error);
            }
