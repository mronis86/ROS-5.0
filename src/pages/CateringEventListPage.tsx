...
const CateringEventListPage: React.FC = () => {
  ...
  const openEvent = (event: EventRow) => {
    navigate(
      `/catering/event?eventId=${encodeURIComponent(event.id)}&eventName=${encodeURIComponent(event.name)}&eventDate=${encodeURIComponent(event.date)}&eventLocation=${encodeURIComponent(event.location)}`
    );
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="text-center mb-4">
          ...
          <p className="text-slate-400 text-sm mt-1 max-w-xl mx-auto">
            {isCateringOnlyUser(user)
              ? 'Select an event to follow the live schedule and catering notes.'
              : 'Open an event to post plating / break notes for the catering team.'}
          </p>
        </div>

        {/* Tabs — centered like Event List */}
        ...
        {/* Search + Refresh — one balanced bar like Event List */}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <input
                  ...
                  className="w-full min-w-0 px-3 py-1.5 bg-slate-700 border border-slate-600 rounded text-white focus:border-blue-500 focus:outline-none text-sm"
                />
              </div>
              <button
                type="button"
              ...
              filtered.map((event) => (
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-orange-300 ml-auto font-semibold">Open →</span>
                  </div>
                </button>
              ))
            )}
                      ...
                      >
                        ...
                        <td className="px-2 py-3 text-center">
                          <button
                            type="button"
                            onClick={() => openEvent(event)}
                            className="px-3 py-1.5 bg-orange-700 hover:bg-orange-600 text-white text-xs font-semibold rounded-lg"
                          >
                            Open
                          </button>
                        </td>
...
export default CateringEventListPage;
