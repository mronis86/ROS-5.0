...
const CommsEventListPage: React.FC = () => {
  ...
  const openEvent = (event: EventRow) => {
    navigate(
      `/comms/event?eventId=${encodeURIComponent(event.id)}&eventName=${encodeURIComponent(event.name)}&eventDate=${encodeURIComponent(event.date)}&eventLocation=${encodeURIComponent(event.location)}`
    );
  };
    ...
    <div className="min-h-screen bg-slate-900 text-white pt-[var(--app-header-height)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="text-center mb-4">
          <h1 className="text-xl sm:text-2xl font-bold text-white">Comms</h1>
          <p className="text-slate-400 text-sm mt-1 max-w-xl mx-auto">
            {isCommsOnlyUser(user)
              ? 'Select an event to mark which cues need to be recorded.'
              : 'Open an event to mark cues for recording. These flags also show on the run of show.'}
          </p>
        </div>
              ...
              filtered.map((event) => (
                ...
                >
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-red-300 ml-auto font-semibold">Open →</span>
                  </div>
                </button>
              ))
            )}
                      ...
                      >
                        ...
                        <td className="px-2 py-3 text-center">
                          <button
                            type="button"
                            onClick={() => openEvent(event)}
                            className="px-3 py-1.5 bg-red-700 hover:bg-red-600 text-white text-xs font-semibold rounded-lg"
                          >
                            Open
                          </button>
                        </td>
...
export default CommsEventListPage;
