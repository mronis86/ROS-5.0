...
const PhotoViewPage: React.FC = () => {
  ...
  // Poll for active timer changes every 1 second (more frequent for smoother updates)
  useEffect(() => {
    if (!event?.id) return;
  ...
  // Separate polling for sub-cue timers (matches Run of Show page)
  useEffect(() => {
    if (!event?.id) return;
    ...
    // Poll every 1 second for smooth sub-cue timer updates (matches Run of Show page)
    const interval = setInterval(syncSubCueTimers, 1000);
    
    return () => clearInterval(interval);
  }, [event?.id, schedule]);

  // Note: Timer progress is updated by database polling every 1 second
  // No need for local timer calculation to avoid false elapsing
