...
const GreenRoomPage: React.FC = () => {
  ...
  const loadActiveTimer = async () => {
    ...
    try {
      ...
      if (activeTimer) {
        ...
        setLoadedItems({ [activeTimer.item_id]: true });
        console.log('🔄 Set loaded items:', { [activeTimer.item_id]: true });
        ...
        if (activeTimer.timer_state === 'running' && activeTimer.started_at) {
          ...
          driftDetector.stopMonitoring(activeTimer.item_id);
          ...
          driftDetector.startMonitoring(
            ...
            (serverElapsed) => {
              ...
              setServerSyncedTimers(prev => {
                ...
                newSet.add(activeTimer.item_id);
                return newSet;
              });
          ...
          try {
            ...
            await driftDetector.forceSync(activeTimer.item_id, async () => {
  ...
  // Poll for active timer changes every 2 seconds
  useEffect(() => {
    if (!event?.id) return;

    loadActiveTimer();
    const interval = setInterval(loadActiveTimer, 2000);
  ...
  // Poll for schedule changes every 20 seconds to check for time updates
  useEffect(() => {
    if (!event?.id) return;

    console.log('🔄 Starting 20-second schedule polling for Green Room...');
    const interval = setInterval(() => {
      console.log('🔄 Polling for schedule changes (every 20s)...');
      loadSchedule();
    }, 20000);

    return () => {
      console.log('🔄 Clearing 20-second schedule polling for Green Room...');
